...
function parseDuration(str: string): number {
  ...
  if (!m)
    throw new Error(`Invalid duration: ${str} (use e.g. 30s, 15m, 2h, 1d)`);
...
async function appendCsvRows(
  ...
  const lines = events.map((e) =>
    columns.map((c) => csvEscape(resolveColumn(e, c))).join(",")
  );
  if (lines.length > 0) {
...
function pickRateLimitWait(headers: Headers): number {
  ...
  const reset =
    headers.get("x-rate-limit-reset") || headers.get("x-ratelimit-reset");
...
async function fetchWithBackoff(
  ...
  while (true) {
    ...
    if (res.status === 429) {
      ...
      console.warn(
        `429 Too Many Requests. Waiting ~${backoff}ms before retry...`
      );
      await sleep(backoff);
      attempt++;
    ...
    if (res.status >= 500) {
      ...
      if (attempt > maxRetries)
        throw new Error(`Server error ${res.status} too many times.`);
      continue;
    }
    if (!res.ok) {
...
async function queryLogs(params: {
  ...
  const { site, apiKey, appKey, query, fromISO, toISO, pageLimit, cursor } =
    params;
  ...
  const rateRemainingStr =
    res.headers.get("x-rate-limit-remaining") ||
    res.headers.get("x-ratelimit-remaining") ||
    "";
  const rateRemaining = rateRemainingStr
    ? parseInt(rateRemainingStr, 10)
    : undefined;
...
async function main(): Promise<void> {
  ...
  const pageLimit = Math.min(
    1000,
    parseInt((argv["page-limit"] as string) || "1000", 10)
  );
  const resume = argv.resume !== "false"; // default true
  const targetPerWindow = parseInt(
    (argv["target-per-window"] as string) || "2000",
    10
  );

  if (!query) throw new Error("--query is required");
  if (columns.length === 0)
    throw new Error("--columns is required (comma separated)");
  ...
  const initialWindow = parseDuration(
    (argv["initial-window"] as string) || "15m"
  );
  ...
  if (!apiKey || !appKey)
    throw new Error(
      "Please set DATADOG_API_KEY and DATADOG_APP_KEY environment variables."
    );
  ...
  while (currentTo > fromMs) {
    ...
    let lastPageIds =
      state?.lastPageIds && Array.isArray(state.lastPageIds)
        ? state.lastPageIds
        : [];
    ...
    while (true) {
      ...
      if (lastPageIds.length && pageEvents.length) {
        ...
        if (before !== pageEvents.length) {
          console.warn(
            `Filtered ${before - pageEvents.length} duplicate events on resume.`
          );
        }
      }
