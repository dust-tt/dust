...
export const defaultExtract = (url: string, root: Element) => {
  const textDOMfFromPage = (root: Element) => {
    type TextDOM = {
    ...
    const textDOMFromPage = (root: Element) => {
    ...
    const tree = textDOMFromPage(root);
...
// URL: ${url}`;
  ...
  return textDOMfFromPage(root);
};

export const extractPage = (url: string) => {
  const defaultExtractor = () => {
    return defaultExtract(url, document.body);
  };
