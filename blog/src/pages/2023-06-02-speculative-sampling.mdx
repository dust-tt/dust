...
And also try it yourself [on this repo](https://github.com/philipperolet/llama-ssp), containing a bit of code Dust released to illustrate the technique.
...
Small models are significantly faster than large ones--e.g. the tests in the [repo](https://github.com/philipperolet/llama-ssp) show the Llama 7B model is ~8 times faster than the 65B.
...
If the draft model is often correct, i.e. the tokens it generates are validated by the target model most of the time, then the gain can be huge. On dumb cases such as completing the prompt "1 2 1 2 1 2 1 2 " etc. you can expect all tokens to be accepted all the time, so you'd get a roughly 4x speed up.
...
The quality remains the same thanks to the *Modified Rejection Sampling* scheme, as the DeepMind paper's author call it. The decision to accept a draft token is made stochastically depending on the ratio of probabilities of this token (target over draft), and on failure the next token is resampled according to accordingly adjusted probabilities. This scheme allows to recover the target model's probability distribution (as proved in the paper's appendix).
...
Anybody can use this sampling strategy, as we hope [the repo shows](https://github.com/philipperolet/llama-ssp). It was made possible by the openness of the ecosystem:
...
Lots of interesting paths to innovate. Crucially, it's essential to find ways to ground the models in truth, ensuring the factuality of their outputs and minimizing potential hallucinations while we forge those paths towards better LLMs. At Dust we look forward to experiment on those to further build productivity tools that make teams smarter.