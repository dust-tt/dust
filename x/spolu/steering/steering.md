...
| 4 | `front/lib/api/assistant/conversation.ts` | Constraints + pending path in `postUserMessage`; add `finalizeGracefulStopAgentMessage()` and `updateAgentMessageWithFinalStatus()` (all locked operations in one file) |
...
2. **Agent loop ends naturally before graceful stop takes effect** — The advisory lock in
   ...
   processed), `updateAgentMessageWithFinalStatus` acts as safety net — it acquires the same advisory
