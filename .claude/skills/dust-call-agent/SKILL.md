...
Use `--projectName` or `--projectId` to create the conversation inside a specific project (space):
...
Use `-d` / `--details` to get detailed message information in the output (e.g. full chain of thought, actions performed by the agent):
