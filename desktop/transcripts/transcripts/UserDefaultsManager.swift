...
class UserDefaultsManager {
    ...
    func saveCredentials(apiKey: String, workspaceId: String, folderId: String) -> Bool {
    ...
    func loadSelectedFolder() -> DustFolder? {
        guard let data = UserDefaults.standard.data(forKey: selectedFolderKey) else {
            return nil
        }
    ...
    func hasCompleteCredentials() -> Bool {
        guard let apiKey = loadAPIKey(), !apiKey.isEmpty,
              let workspaceId = loadWorkspaceId(), !workspaceId.isEmpty else {
            return false
        }
        return true
    }
