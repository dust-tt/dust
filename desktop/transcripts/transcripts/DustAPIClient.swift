...
class DustAPIClient {
    ...
    private func createAuthenticatedRequest(for url: URL, apiKey: String, workspaceId: String) -> URLRequest {
    ...
    func fetchSpaces(apiKey: String, workspaceId: String) async throws -> [DustSpace] {
        let urlString = "\(baseURL)/w/\(workspaceId)/spaces"
        guard let url = URL(string: urlString) else {
            throw DustAPIError(message: "Invalid URL: \(urlString)", statusCode: nil)
        }
        
        let request = createAuthenticatedRequest(for: url, apiKey: apiKey, workspaceId: workspaceId)
        ...
        do {
            ...
            guard httpResponse.statusCode == 200 else {
                ...
                throw DustAPIError(message: "HTTP \(httpResponse.statusCode)", statusCode: httpResponse.statusCode)
            }
            
            let spacesResponse = try JSONDecoder().decode(DustSpacesResponse.self, from: data)
    ...
    func fetchDataSourceViews(apiKey: String, workspaceId: String, spaceId: String) async throws -> [DustDataSourceView] {
        let urlString = "\(baseURL)/w/\(workspaceId)/spaces/\(spaceId)/data-source-views"
        guard let url = URL(string: urlString) else {
            throw DustAPIError(message: "Invalid URL: \(urlString)", statusCode: nil)
        }
        ...
        do {
            ...
            let dataSourceViewsResponse = try JSONDecoder().decode(DustDataSourceViewsResponse.self, from: data)
            // Filter to only return manual data sources (where connectorProvider is null)
            return dataSourceViewsResponse.dataSourceViews.filter { $0.isManualDataSource }
    ...
    func fetchAvailableFolders(apiKey: String, workspaceId: String) async throws -> [DustFolder] {
        ...
        for space in spaces {
            do {
                ...
                print("Failed to fetch data source views for space \(space.name): \(error)")
            }
        }
