...
import { HttpMcpTransport } from "../../mcp/types/transports/http.js";
...
const AgentsMCP: FC<AgentsMCPProps> = ({ port, sId: requestedSIds }) => {
  ...
  useEffect(() => {
    if (confirmedSelection && !isServerStarted) {
      const service = new AgentsMcpService(
        new HttpMcpTransport(port),
        confirmedSelection
      );
      ...
      const start = async () => {
        try {
          const serverUrl = await service.startServer();
