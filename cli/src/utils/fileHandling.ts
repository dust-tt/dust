...
/**
 * Parse file paths from command line arguments or input
 * Only extracts tokens that look like file paths
 */
export function parseFilePaths(input: string): string[] {
  ...
  // Only return tokens that actually look like file paths
  return tokens.filter((token) => looksLikeFilePath(token));
}
...
/**
 * Check if path looks like a file path (has extension or specific patterns)
 */
export function looksLikeFilePath(input: string): boolean {
  // Must be a reasonable length for a file path
  ...
  // Check for relative path indicators with reasonable path structure
  if (input.startsWith("./") || input.startsWith("../")) {
    // Must have more than just the prefix and not contain spaces (unless quoted)
    return input.length > 2 && !input.includes(" ");
  }
  ...
  // Check for absolute paths (but be more restrictive)
  if (input.startsWith("/") && input.length > 1) {
    // Must look like a real path, not just contain a slash
    return (
      !input.includes(" ") &&
