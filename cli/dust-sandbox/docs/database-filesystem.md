...
- Linux-targeted Rust filesystem tests: 29 passing, including token/cache
...
| --- | ---: | ---: | ---: |
| Cold read 4 KiB | 2,033.7 ms | 305.7 ms | 0.15x |
| Cold read 1 MiB | 2,529.2 ms | 710.5 ms | 0.28x |
| Cold read 8 MiB | 2,151.9 ms | 985.1 ms | 0.46x |
| Warm read 4 KiB | 1,742.2 ms | 0.32 ms | <0.01x |
| Warm read 1 MiB | 1,759.2 ms | 1.96 ms | <0.01x |
| Warm read 8 MiB | 1,736.0 ms | 10.75 ms | <0.01x |
| `stat` | 559.5 ms | 0.01 ms | <0.01x |
| `readdir` with 20 entries | 323.3 ms | 0.31 ms | <0.01x |
| Write + `fsync`, 4 KiB | 1,038.1 ms | 874.6 ms | 0.84x |
| Write + `fsync`, 1 MiB | 1,625.4 ms | 1,335.8 ms | 0.82x |
| Write + `fsync`, 8 MiB | 2,512.9 ms | 1,838.8 ms | 0.73x |
| Create | 945.5 ms | 68.1 ms | 0.07x |
| Rename | 2,007.2 ms | 217.2 ms | 0.11x |
| Unlink | 978.9 ms | 153.0 ms | 0.16x |
| Conversation-to-Pod move | 4,208.7 ms | 229.0 ms | 0.05x |
...
`fsync` took 10.21 s on gcsfuse and 5.22 s on Dust. Reading and checking those
eight files took 4.79 s on gcsfuse and 0.19 s on Dust. Peak filesystem-process
RSS was 211.3 MiB for two gcsfuse processes and 12.1 MiB for one Dust process.
...
them after 155–1,090 ms, with a 918 ms median. A new non-empty file appeared
after 1,210 ms with revision 1. A rename appeared after 19 ms and kept the same
inode and revision. A deletion appeared after 482 ms.
...
poll, 298 ms and 460 ms after the winning commit. These delays include the
...
The complete acceptance workload also passed with `--cache-capacity-mib 0`.
This forces every closed content entry out of the local cache and checks that
an open file remains pinned until its handle closes.
