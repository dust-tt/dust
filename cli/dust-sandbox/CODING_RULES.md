...
### [TEST1] Tests live inline

Tests go in the same file as the code they test, using `#[cfg(test)] mod tests`.

## CLI

### [CLI1] One command per file

Each CLI subcommand lives in its own file under `src/commands/`. Re-export the handler from `commands/mod.rs`.
