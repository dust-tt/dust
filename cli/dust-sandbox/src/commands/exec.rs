...
pub async fn cmd_exec(
    ...
    if tool.is_none() {
        let available: Vec<&str> = resolved.server.tools.iter().map(|t| t.name.as_str()).collect();
        bail!(
...
mod tests {
    ...
    #[test]
    fn parse_empty_args() {
        let result = parse_args(&[]).expect("should parse empty");
    ...
    fn parse_string_args() {
        ...
        let result = parse_args(&args).expect("should parse").expect("should have value");
