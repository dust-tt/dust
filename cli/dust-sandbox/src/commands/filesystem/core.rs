...
impl DustFilesystem {
    ...
    pub fn mkdir(&self, parent_inode: u64, name: &OsStr) -> FsResult<Entry> {
        ...
        // Directory markers do not carry Dust identity. Let the lower gcsfuse
        // mount own the operation directly instead of paying for a Front
        // round-trip and journal row.
        fs::create_dir(&node.resolved_path)?;
    ...
    pub fn rmdir(&self, parent_inode: u64, name: &OsStr) -> FsResult<()> {
        ...
        // A directory must be empty here, so it cannot contain a Frame binding.
        // The lower gcsfuse mount can remove it without application mediation.
        fs::remove_dir(&node.resolved_path)?;
    ...
    pub fn open(&self, inode: u64, flags: i32) -> FsResult<u64> {
        ...
        self.insert_handle(file, mount_index, path)
    }
    ...
    pub fn create(
        ...
        let handle = self.insert_handle(file, mount_index, path)?;
        Ok((entry, handle))
    }
    ...
    fn insert_handle(&self, file: File, mount_index: usize, path: PathBuf) -> FsResult<u64> {
        ...
        self.handles.lock().insert(
            ...
            OpenHandle {
                file,
                state: HandleState { mount_index, path },
            },
        );
        Ok(handle)
    }
...
pub fn run_self_test() -> anyhow::Result<()> {
    ...
    anyhow::ensure!(
        operations.len() == 3,
        "expected three identity-sensitive operations, got {}",
        operations.len()
    );
    anyhow::ensure!(
        matches!(operations[1].1, MutationOperation::Rename { .. }),
        "second semantic operation should be the cross-mount rename"
    );
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn writes_do_not_call_the_semantic_mutation_api() {
        let temporary_directory = tempfile::tempdir().expect("temporary directory should exist");
        ...
        filesystem.release(handle).expect("release should succeed");
        assert!(adapter.operations.lock().is_empty());
    }
