...
impl Drop for H2ConnectionLease {
    fn drop(&mut self) {
        ...
        self.connection
            .last_used_ms
            .store(pool_now_millis(), Ordering::SeqCst);
    }
}
...
impl H2UpstreamPool {
    pub(super) fn new(open_upstream: OpenUpstream) -> Self {
        Self {
            inner: Arc::new(H2UpstreamPoolInner {
                open_upstream,
...
async fn insert_h2_connection_locked(
    ...
    let pooled = Arc::new(PooledH2Connection {
        ...
        last_used_ms: AtomicU64::new(pool_now_millis()),
    });
    let connection_for_task = Arc::clone(&pooled);
    tokio::spawn(async move {
        if let Err(error) = connection.await {
            warn!(error = %error, "outbound h2 upstream connection closed with error");
        }
        mark_h2_connection_closed(&connection_for_task);
    });
...
fn prune_idle_pool_entries(entries: &mut HashMap<H2UpstreamKey, Vec<Arc<PooledH2Connection>>>) {
    let now_ms = pool_now_millis();
    entries.retain(|_, connections| {
        connections.retain(|connection| {
            is_h2_connection_live(connection) && !connection_is_idle_expired(connection, now_ms)
        });
