...
describe("CLI end-to-end", () => {
  test("reads request JSON from stdin and writes response JSON to stdout", async () => {
    const input = JSON.stringify({ url: "http://localhost/?name=cli" });
    const proc = Bun.spawn(
      ["bun", join(import.meta.dir, "run_request.ts"), fixture("hello.ts")],
  ...
  test("resolves a relative handler path against the working directory", async () => {
    const input = JSON.stringify({ url: "http://localhost/?name=rel" });
    const proc = Bun.spawn(
      ["bun", join(import.meta.dir, "run_request.ts"), "hello.ts"],
      {
    ...
    const out = JSON.parse(stdout);
    expect(out.ok).toBe(true);
    expect(JSON.parse(out.response.body)).toEqual({ hello: "rel" });
  });

  test("exits nonzero and emits ok:false JSON when the handler throws", async () => {
    const input = JSON.stringify({ url: "http://localhost/" });
    const proc = Bun.spawn(
      ["bun", join(import.meta.dir, "run_request.ts"), fixture("throws.ts")],
      { stdin: new TextEncoder().encode(input), stdout: "pipe", stderr: "pipe" }
    );
    const stdout = await new Response(proc.stdout).text();
    const code = await proc.exited;
