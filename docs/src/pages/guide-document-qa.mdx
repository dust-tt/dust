...
This document assumes that you've already created a Dust app and that you have already set up OpenAI
as a provider on your Dust account. If you haven't done so, please refer to our
...
The main limitation of LLMs when it comes to answering question about long documents is that the
...
able to insert 12 chunks in-context and leave 1024 tokens for genreating an answer as we plan to use
`gpt-3.5-turbo` which has a 4096 tokens context size.
...
tokens appears like a good trade-off for our use-case but the right value will depend on your
...
```txt
You are an helpful assistant. For each user question, you'll receive 8 additional system
...
You can now run your app and inspec the outputs of the last `code` block.
