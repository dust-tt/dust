...
A conversation is consituted of `UserMessage` and `AgentMessages`. A `UserMessage` is a message sent
by a human to interact with one or more assistants. An `AgentMessage` is an assistant response.
`AgentMessages` can potentially include an action. Currently the only action supported is a
`RetrievalAction` which triggers for agents that are configured to perform a retrieval before
streaming their response.
...
<Properties>
  ...
  <Property name="action" type="object | null">
    ...
    generating an answer. Currently only `RetrievalAction` are supported.
  </Property>
  <Property name="content" type="string">
...
<Row>
  <Col>
    ...
    returned upon creation before posting the user message to it.
