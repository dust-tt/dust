...
Dust apps rely on models providers to interact with large language models. You can setup your first
...
<Image src="/quickstart_openai_provider.png" alt="OpenAI provider setup">
  ...
  enabled in the **Providers** pane._
</Image>

Whenever your app will interact with an OpenAI model, it'll use the API key you provided to do so.
The costs of running these models will be reported on OpenAI's dashboard.

## Creating a new app

You can now create a new app by clicking on the **New App** button from your main dashboard. You'll
...
<Note>
  Public apps are visible to everyone, private apps are solely accessible to
  you. Public apps can be cloned by others, but cannot be run by anyone else
  than you.
</Note>
...
let you manage datasets (for inputs or few-shot examples).

The **Run** button is currently disabled as you haven't added any block to your app yet.

## Add your first block

You are ready to add your first block. Click on the **+ Block** button and add an `input` block. The
...
Click on the **Create Dataset** button inside the `input` block to create a new dataset. You are now
redirected to the **Datasets** pane. And a new draft dataset with basic arithmetic questions has
been prepared for you. Give the name _dev_ to your dataset, you can leave the description empty. Our
example app will attempt to answer these basic arithmetic questions. The **Schema** lets you control
the schema of your dataset's entries. The Data section lets you edit, add, or remove elements to
your dataset. You can leave the dataset as is and click **Create**.
...
<Note>
  ...
  your `data` block). Know that you can change the name of your blocks by
  clicking on the block name in the specification view. Let's keep the default
  name for now so that we can refer to it in the next blocks.
</Note>
...
<Image src="/quickstart_data.png" alt="Execution of the data block">
  ...
  as a different behaviour than the `input` block. For each execution stream it
...
Now is time to construct our prompt. The prompt field supports [Tera](https://tera.netlify.app/)
...
We will use the following prompt to encode to the model (using few-shot prompting) that we want it
...
This is called few-shot prompting, by showing a few valid examples, we encode the task to the model.
...
try to complete the prompt with more questions and answers after genreting the answer to your
question.
...
<Note>
  ...
  `llm` block will quick in and you won't spend any resources on OpenAI.
</Note>
...
to create a new API key in the **API Keys** panel. Click the **Create new secret key** button and
