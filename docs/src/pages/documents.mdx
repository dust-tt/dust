...
<Row>
  <Col>
    ...
    <Properties>
      ...
      <Property name="tags_not_in" type="array[string]">
        Exclude documents tagged with one of these tags from the search results. You can specify multiple tags_not_in in the search query.
      </Property>            
    </Properties>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl "https://dust.tt/api/v1/data_sources/spolu/foo/search?query=secret&top_k=10&full_text=true&tags_in=foo&tags_in=bar&tags_not_in=baz&tags_not_in=zab" \
      -H "Authorization: Bearer sk-..."
    ```
