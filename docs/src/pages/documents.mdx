...
## Search Documents {{ tag: 'POST', label: '/v1/data_sources/:user/:data_source_name/search' }}
...
<Row>
  <Col>
    ...
    <Properties>
      ...
      <Property name="full_text" type="boolean">
        Whether or not ot return the full text associated with the matching chunk
      </Property>


      ### Optional JSON body attributes
      <Property name="filter" type="object">
      All fields in the following filter data structure are optionals
      ```json
      {
        "timestamp":{"gt":1679447275021, "lt":1679437275024},
        "tags":{"in":["tag1", "tag2"], "not":["tagx", "tagy"]}
      }
      ```
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl "https://dust.tt/api/v1/data_sources/spolu/foo/search" \
      -H "Authorization: Bearer sk-..."
    ```
