...
block](core-blocks#data-source-block) which, given a query, automatically embeds it and performs a
vector search operation to retrieve the most semantically relevant documents and associated chunks.
...
Conversely, if you plan to process the retrieved documents with subsequent calls to models (let say
using recursive summarization or such) then you want to pick a higher chunk size such that the
semantic retrieval step covers as much information as possible.

These decisions really depend on the quality and speed constraint you have to design your app and we
advise that you experiment with various approach to find the one that suits your use-case the most.
...
- **Embedding**: The query is embedded using the embedding model configured on the data source.
