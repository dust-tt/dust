...
Integration blocks are [Blocks](/overview#blocks) that are used to integrate with external systems,
mostly for retrieving data. They can be used to search the web, retrieve content or more generally
make API calls.
...
[SerpApi](https://serpapi.com/) (planning to add Google Programmable Search Engine). It requires
