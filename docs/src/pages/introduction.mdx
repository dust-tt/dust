...
These models that are trained to embed text into a vector space. Their input is a piece of text and
their output is a high-dimensional vector of floats (hundreds to thousands). The resulting
embeddings represent the text semantically and can be used to perform search.
...
to discuss the same concept (say "dog" and "canine").

The process of retrieving highly similar vectors and their associated chunks given a query (and its
vector) is called vector search.

See [data sources](/data-sources) to learn more about how Dust provide fully-managed solutions to
automatically embed documents and perform vector search on the resulting database.
...
here: [recusive-summarization](https://dust.tt/spolu/a/eeac37ce40).
