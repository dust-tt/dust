...
embedded using the embedding model configured on the data source and the resulting enmbedding vector
...
objects. Each document may include one or more [Chunks](/documents#the-chunk-model) (only the ones
that were returned from the search). The documents are ordered by the max of their retrieved chunks
score. Please refer to the [data sources overview](/data-sources-overview) for more details about
how documents are chunked to enable semantic search.
