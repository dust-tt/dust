...
The `data_source` block provides an interface to query a data source and semantically similar chunks
to the query provided. When executed, the query is embedded using the embedding model configured on
the data source and the resulting enmbedding vector is used to perform semantic search on the data
source's chunks.

The output of the `data_source` block is a list of _Document_ objects. Each document may include one
or more _Chunks_ (only the ones that were returned from the search). The _Documents_ are ordered by
the max of their retrieved chunks score. Please refer to the [Data Sources
overview](/data-sources-overview#querying-a-data-source) for more details on the properties of the
_Document_ and _Chunk_ objects.
...
<Properties>
  <Property name="query" type="string">
    ...
    [Tera](https://tera.netlify.app/docs/#templates) templating language (see
    the [LLM block](/core-blocks#llm-block) documentation for more detail on
    templating).
  </Property>
...
<Properties>
  <Property name="project_id" type="string">
    Internal `project_id` associated with the data source. The Dust's interface
    presents a data source picker that lets you select your own or other users'
    data sources without having to explecitely know the associated `project_id`.
  </Property>
  <Property name="data_source_id" type="string">
    The name of the data source to use in the `project_id` above.
  </Property>
</Properties>
