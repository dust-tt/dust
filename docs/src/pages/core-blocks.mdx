...
API](https://platform.openai.com/docs/guides/chat)). When these models are used, as part of the
`llm` block, `max_tokens` and `top_logprobs` are ignored, and the content of the templated prompt is
passed as initial message with _user_ role. The block response is the content of the `assistant`
role message returned by the model. Note that logprobs and tokens are not available with these
models.
...
OpenAI's `gpt-3.5-turbo` ([Chat API](https://platform.openai.com/docs/guides/chat)). It is
similar in many respects to an LLM block except that it does not have `max_tokens`, has templated
(see LLM block above) intstructions that serve as initial prompt and exposes a `messages` code block
to output previous messages (generally passed to the app as input)

It is used to build chat-based experiences where the model is exposed with previous interactions.
