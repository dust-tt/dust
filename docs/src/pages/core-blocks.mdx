...
produce ouptputs and can refer to the ouptuts of previously executed blocks. We invite you to review
...
The `input` block is the entry point to a Dust app and receives the arguments, in the form of JSON
objects, on which the app is executed. During the design phase of the app it is associated to a
...
Refer to the [Inputs](/overview#inputs) section for more detail on how the `input` block forks the
...
The `data` block outputs the dataset it is associated with. Conversely to the `input` block it does
not fork the execution of the app and simply returns the entire dataset as an array of JSON objects.
...
<Properties>
  ...
  <Property name="config" type="object">
    ...
    are block names and values are their associated configuration values.
  </Property>
</Properties>
...
<Properties>
  ...
  <Property name="temperature" type="float">
    The temperature to use when sampling from the model. Higher _temperature_
    (eg `1.0`) results in more diverse completions, while lower _temperature_
    (eg `0.0`) results in more conservative completions.
  </Property>
  <Property name="max_tokens" type="integer">
    The maximum number of tokens to generate from the model. A can very broadly
    seen as a word worth of content. The model may decide to stop generation
    before reaching `max_tokens`.
  </Property>
  <Property name="stop" type="[]string">
    An array of strings that should interupt completion when sampled from the
    model.
  </Property>
  ...
  <Property name="top_logprobs" type="integer">
    ...
    well as the chosen token. For example, if logprobs is 5, the API will return
    a list of the 5 most likely tokens' probabilities. Useful for classification
    tasks.
  </Property>
</Properties>
...
<Properties>
  ...
  <Property name="use_cache" type="bool">
    ...
    variables, then the cached response is returned.
  </Property>
...
[Tera](https://tera.netlify.app/docs/#templates). This is in particular useful to construct few-shot
...
app relying on this prompt can be found [here](https://dust.tt/spolu/a/d903a92151).

The [Tera](https://tera.netlify.app/) templating language supports a variety of constructs including
loops and variable replacement. We refer to the [Tera
...
of an array, in parallel. The `map` block takes as `from` specification argument a block name. If
