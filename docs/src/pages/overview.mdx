...
Each block have a set of _specification_ arguments and _configuration_ arguments. The specification
...
an app with various configurations, as an example, to evaluate the performance of different models.
To illustrate the distinction, the `llm` block's _prompt_ is part of its specification whereas the
_model_ is part of its configuration.

The Dust Platform UI does not explicitely expose the distinction. But you can arbitrarily change the
