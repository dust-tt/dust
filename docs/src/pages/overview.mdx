...
# Dust Developer Platform overview
...
Dust apps can use developer secrets to store sensitive information such as API keys or passwords.
Developer secrets are stored encrypted and are only accessible to the app they are associated with.

Currently, only the `cURL` block can use developer secrets to authenticate to external services.
...
[Data sources](/data-sources) are fully managed semantically searchable databases of
