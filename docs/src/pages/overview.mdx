...
well as the overall orchestration logic. This iterative nature makes it easy to forget or fail
to capture the state of an app at a given point in time. This prevents its recovery if the design
...
Dust apps are composed of blocks executed sequentially. Blocks don't have inputs, but they
...
an app with various configurations. As an example, this can be used to evaluate the performance of different models. To illustrate the distinction, the `llm` block's _prompt_ is part of its specification whereas the _model_ is part of its configuration.
...
<Note>
  The `input` block is used to parametrize an app's behavior. While designing the
  app, it is associated to a dataset of input examples. When the app is run by API, 
  it will return the inputs passed by API.
</Note>
