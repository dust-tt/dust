...
ARG COMMIT_HASH_LONG
ARG DATADOG_API_KEY
ARG DUST_CLIENT_FACING_URL
ARG DUST_APP_URL
ARG GTM_TRACKING_ID
ARG ENABLE_BOT_CRAWLING
ARG DATADOG_CLIENT_TOKEN
ARG DATADOG_SERVICE
ARG POSTHOG_KEY
ARG VIRTUOSO_LICENSE_KEY
ARG NOVU_APPLICATION_IDENTIFIER
ARG NOVU_API_URL
ARG NOVU_WEBSOCKET_API_URL
ARG BUILD_DATE
...
ENV DUST_CLIENT_FACING_URL=$DUST_CLIENT_FACING_URL
ENV DUST_APP_URL=$DUST_APP_URL
ENV GTM_TRACKING_ID=$GTM_TRACKING_ID
ENV ENABLE_BOT_CRAWLING=$ENABLE_BOT_CRAWLING
ENV DATADOG_CLIENT_TOKEN=$DATADOG_CLIENT_TOKEN
ENV DATADOG_SERVICE=$DATADOG_SERVICE
ENV POSTHOG_KEY=$POSTHOG_KEY
ENV VIRTUOSO_LICENSE_KEY=$VIRTUOSO_LICENSE_KEY
ENV NOVU_APPLICATION_IDENTIFIER=$NOVU_APPLICATION_IDENTIFIER
ENV NOVU_API_URL=$NOVU_API_URL
ENV NOVU_WEBSOCKET_API_URL=$NOVU_WEBSOCKET_API_URL
...
RUN --mount=type=cache,id=next-cache,target=/app/front/.next/cache \
  ...
  if [ -n "$DATADOG_API_KEY" ] && [ -n "$DATADOG_SERVICE" ]; then \
  ...
  --service=$DATADOG_SERVICE-browser && \
