...
import { runScheduledAgentsQueueWorker } from "@app/temporal/scheduled_agents/worker";
...
runScheduledAgentsQueueWorker().catch((err) =>
  logger.error({ error: err }, "Error running scheduled agents queue worker.")
);
