...
export default function TogetherAISetup({
  owner,
  config,
  enabled,
}: {
  ...
  const runTest = async () => {
    setTestRunning(true);
    setTestError("");
    const check = await checkProvider(owner, "togetherai", {
      api_key: apiKey,
    });
  ...
  const handleEnable = async () => {
    ...
    const res = await fetch(`/api/w/${owner.sId}/providers/togetherai`, {
      headers: {
  ...
  return (
    <NewDialog>
      ...
      <NewDialogContent>
        <NewDialogHeader>
          <NewDialogTitle>Setup TogetherAI</NewDialogTitle>
          <NewDialogDescription>
            To use TogetherAI models you must provide your API key. We'll never
            ...
            <div className="text-sm">
              {testError ? (
                ...
                <span className="text-green-600">
                  Test succeeded! You can enable TogetherAI.
                </span>
              ) : (
                <span>&nbsp;</span>
              )}
