...
export default function GoogleAiStudioSetup({
  owner,
  config,
  enabled,
}: {
  ...
  const runTest = async () => {
    setTestRunning(true);
    setTestError("");
    const check = await checkProvider(owner, "google_ai_studio", {
      api_key: apiKey,
    });
  ...
  const handleEnable = async () => {
    ...
    const res = await fetch(`/api/w/${owner.sId}/providers/google_ai_studio`, {
      headers: {
  ...
  return (
    <NewDialog>
      ...
      <NewDialogContent>
        <NewDialogHeader>
          <NewDialogTitle>Setup Google AI Studio</NewDialogTitle>
          <NewDialogDescription>
            <div className="mt-4">
              <p>
                To use Google AI Studio models you must provide your API key. It
                can be found{" "}
                <a
