...
export default function MistralAISetup({
  owner,
  open,
  setOpen,
  config,
  ...
  const runTest = async () => {
    setTestRunning(true);
    setTestError("");
    const check = await checkProvider(owner, "mistralai", { api_key: apiKey });
  ...
  const handleEnable = async () => {
    ...
    const res = await fetch(`/api/w/${owner.sId}/providers/mistralai`, {
      headers: {
