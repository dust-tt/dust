...
export function triggerTypeToBuilderType(
  trigger: TriggerType
): AgentBuilderTriggerType {
  switch (trigger.kind) {
    case "schedule":
...
export function findWebhookSourceViewForTrigger(
  trigger: TriggerType,
  webhookSourceViews: WebhookSourceViewType[]
): WebhookSourceViewType | null {
  if (trigger.kind !== "webhook" || !trigger.webhookSourceViewSId) {
    return null;
  }
  return (
    webhookSourceViews.find((v) => v.sId === trigger.webhookSourceViewSId) ??
    null
  );
}
