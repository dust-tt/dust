...
const keyId = (workspaceId: string, userId: string, conversationId: string) =>
...
export function useConversationDrafts({
  ...
  const getDraftsFromStorage = useCallback((): DraftStorage => {
    try {
      ...
      Object.entries(parsed).forEach(([id, draft]) => {
        if (now - draft.timestamp < expiryMs) {
          cleaned[id] = draft;
        }
      });
  ...
  const debouncedSave = useRef(
    debounce(
      ({
        ...
        drafts[keyId(workspaceId, userId, conversationId)] = {
          text,
          timestamp,
        };
  ...
  const getDraft = useCallback((): ConversationDraft | null => {
    const drafts = getDraftsFromStorage();
    return (
      drafts[keyId(workspaceId, internalUserId, internalConversationId)] ?? null
    );
  }, [
  ...
  const clearDraft = useCallback(() => {
    const drafts = getDraftsFromStorage();
    delete drafts[keyId(workspaceId, internalUserId, internalConversationId)];
    saveDraftsToStorage(drafts);
  ...
  return useMemo(
    () => ({ saveDraft, getDraft, clearDraft, clearAllDraftsFromUser }),
    [saveDraft, getDraft, clearDraft, clearAllDraftsFromUser]
  );
}
