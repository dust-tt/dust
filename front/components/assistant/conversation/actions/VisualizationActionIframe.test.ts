...
describe("Frame file RPC", () => {
  it("keeps getFile unchanged and returns revisions only through readFile", async () => {
      ...
      expect(postMessage).toHaveBeenCalledWith(
        {
          ...
          identifier: "viz-fil_frame",
          messageUniqueId: "legacy-read",
          result: { fileBlob: expect.any(Blob) },
        },
        { targetOrigin: "*" }
      )
    );
