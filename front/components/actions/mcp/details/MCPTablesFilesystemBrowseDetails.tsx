...
import { isTablesFilesystemBrowseType } from "@app/lib/actions/mcp_internal_actions/output_schemas";
import { getDocumentIcon } from "@app/lib/content_nodes";

export function TablesFilesystemBrowseDetails({
  action,
  defaultOpen,
}: MCPActionDetailsProps) {
  const browseResult = action.output
    ?.filter(isTablesFilesystemBrowseType)
    .map((o) => o.resource)?.[0];
  ...
  const { nodeId, data, text } = browseResult;

  return (
    <ActionDetailsWrapper
      actionName="Browse tables"
