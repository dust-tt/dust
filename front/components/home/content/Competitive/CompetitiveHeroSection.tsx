...
import { CheckV2, ClockV2, Icon, Lock01V2 } from "@dust-tt/sparkle";
                  ...
                  {valueProps.map((prop, index) => (
                    <li key={index} className="flex items-start gap-3">
                      <span className="mt-0.5 flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-emerald-400">
                        <CheckV2 className="h-3 w-3 text-white" />
                      </span>
          ...
          <div className="flex flex-wrap items-center justify-center gap-6 text-sm text-muted-foreground">
            {trustBadges.map((badge, index) => (
              <div key={index} className="flex items-center gap-2">
                {index === 0 ? (
                  <Icon visual={CheckV2} className="h-4 w-4 text-emerald-500" />
                ) : index === 1 ? (
                  <Icon visual={ClockV2} className="h-4 w-4 text-blue-500" />
                ) : (
                  <Icon visual={Lock01V2} className="h-4 w-4 text-amber-500" />
                )}
                <span>{badge}</span>
              </div>
            ))}
