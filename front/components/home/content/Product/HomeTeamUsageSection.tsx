  ...
  return (
    <section className="w-full bg-background py-24">
      <div className="mx-auto flex w-full max-w-[1180px] flex-col gap-12 px-6">
        ...
        <Tabs defaultValue={TABS[0].id} className="flex flex-col gap-10">
          ...
          {TABS.map((tab, index) => (
            <TabsContent
              key={tab.id}
              value={tab.id}
              className="data-[state=inactive]:hidden grid grid-cols-1 gap-10 data-[state=active]:animate-in data-[state=active]:fade-in-0 data-[state=active]:duration-200 lg:grid-cols-2 lg:gap-16"
            >
              <div
                className={`relative flex aspect-[4/3] w-full items-center justify-center self-stretch overflow-hidden rounded-2xl lg:aspect-auto lg:min-h-[480px] ${tab.bg}`}
              >
                <Image
                  src={tab.imageSrc}
                  alt={tab.imageAlt}
