...
export default function SearchPicker({ user, model, readOnly, onModelUpdate }) {
  ...
  let searchProviders = filterServiceProviders(providers);
  ...
  if (
    ...
    searchProviders.filter((p) => p.providerId == model.provider_id).length == 0
  ) {
    setTimeout(() => {
      onModelUpdate({
        provider_id: "",
