...
export const useSkillSelection = ({
  ...
  // IDs of skills that were already added before opening the sheet
  const alreadyAddedSkillIds = useMemo(
    () => new Set(initialSelectedSkills.map((s) => s.sId)),
    [initialSelectedSkills]
  );

  // IDs of skills selected in this session (newly selected, not already added)
  const selectedSkillIds = useMemo(
    () =>
      new Set(
        localSelectedSkills
          .filter((s) => !alreadyAddedSkillIds.has(s.sId))
          .map((s) => s.sId)
      ),
    [localSelectedSkills, alreadyAddedSkillIds]
  );

  const filteredSkills = useMemo(() => {
    // Filter out already-added skills (same pattern as tools)
    let skills = skillsWithRelations.filter(
