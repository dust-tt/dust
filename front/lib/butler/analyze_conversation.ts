...
const ANALYZE_CONVERSATION_FUNCTION_NAME = "analyze_conversation";
...
function buildAnalyzeConversationSpecifications(
  ...
  return [
    {
      name: ANALYZE_CONVERSATION_FUNCTION_NAME,
      description:
        "Analyze the conversation to evaluate whether the title should be updated " +
...
function buildPrompt(
  ...
  let prompt =
    "You are a conversation analyst. Your job is to:\n" +
...
async function shouldProposeRenameTitle(
  ...
  // If the source message was deleted, allow a new suggestion.
...
export async function analyzeConversation(
  ...
  const res = await runMultiActionsAgent(
    auth,
    {
      ...
      functionCall: ANALYZE_CONVERSATION_FUNCTION_NAME,
      useCache: false,
    },
    {
      conversation: conv,
      ...
      forceToolCall: ANALYZE_CONVERSATION_FUNCTION_NAME,
    },
    {
      context: {
  ...
  if (!action?.arguments) {
    logger.warn(
      { conversationId: conversation.id },
      "Butler: no tool call in LLM response for conversation analysis"
    );
    return;
  }
  ...
  if (agent_confidence >= 70 && agent_name && agent_prompt) {
