...
import type { ConsumptionScopeFilterKey } from "@app/lib/api/analytics/consumption/scope";
import {
...
// The URL borrows the filter vocabulary the API already uses, so there is no
// second set of names to keep stable. A name that has shipped inside a link is
// never repurposed.
const CATEGORY_PARAM = {
  ...
  source: "sources",
  api_key: "api_keys",
} as const satisfies Record<UsageFilterCategory, ConsumptionScopeFilterKey>;
...
export function analyticsViewQueryString(query: AnalyticsQuery): string {
  ...
  for (const name of ANALYTICS_VIEW_PARAMS) {
