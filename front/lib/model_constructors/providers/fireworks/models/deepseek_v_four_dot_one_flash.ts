...
import { DEEPSEEK_V4P1_FLASH } from "@app/lib/model_constructors/types/models";
import { z } from "zod";
...
// Fireworks also accepts medium/xhigh for any model it serves, but DeepSeek
// documents mapping those onto `high`, which would silently rewrite the
// caller's choice; excluding them is policy (follow the model author), not a
// provider constraint.
...
>(Base: TBase) {
  ...
  return DeepSeekDeepSeekV41Flash;
}
