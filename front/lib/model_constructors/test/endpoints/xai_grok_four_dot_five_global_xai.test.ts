...
export const XaiGrokFourDotFiveGlobalXaiStreamSetup: StreamSetup = {
  ...
  // `null` runs the case with its default checkers; a checker array
  // overrides them. Every case always runs.
  //
  // Verified against the live xAI API (2026-07-27) with the widest
  // `inputConfigSchema`. `none` and `maximal` are hard-rejected by the API;
  // `minimal` and `xhigh` are accepted by the API but not documented for
  // grok-4.5, so the schema excludes them on purpose (see the endpoint file).
  // All four are configuration errors here. Temperature is allowed.
  tests: {
