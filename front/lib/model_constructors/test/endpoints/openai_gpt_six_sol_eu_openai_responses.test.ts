...
export const OpenAIGptSixSolEuropeOpenAIResponsesStreamSetup: StreamSetup = {
  createInstance: () =>
    new OpenAIGptSixSolEuropeOpenAIResponsesStream({
  ...
  // none/low/medium/high/xhigh/max, and `temperature` is only accepted with
  // effort "none". Global and EU rejected exactly the same cases.
...
// NODE_ENV=test RUN_LLM_TEST=true npm run test -- --config lib/model_constructors/test/vite.config.js --bail 1 lib/model_constructors/test/endpoints/openai_gpt_six_sol_eu_openai_responses.test.ts
