...
export async function mergeConversationTodosIntoProject(
  auth: Authenticator,
  // spaceId is the string sId; rename internally to avoid shadowing the
  // numeric spaceId extracted from the resolved resource below.
  { spaceId: spaceSId }: { spaceId: string }
): Promise<void> {
  ...
  // 5. Auto-done: mark todos (regardless of who created them) as done when
