...
describe("skill ranking", () => {
  it.each([
    ...
    ["istan", "İstanbul", "", 60],
    ["ΟΣ", "οσ", "", 60],
  ])("scores %j against code-defined name %j", (searchTerm, name, description, expected) => {
    expect(
      getSkillSearchScore({ searchTerm, name, description, mode: "discovery" })
    ).toBe(expected);
  });

  it("uses the whole-name analyzer for the exact-match boost", () => {
    expect(
      buildSkillMatchQuery("resume de reunion").dis_max?.queries[0]
    ).toEqual({
      constant_score: {
        filter: { match: { name: "resume de reunion" } },
        boost: 100,
      },
    });
  });

  it("searches split and preserved search_as_you_type fields together", () => {
    expect(buildSkillMatchQuery("report b").dis_max?.queries[2]).toEqual({
      constant_score: {
        filter: {
          multi_match: {
            query: "report b",
