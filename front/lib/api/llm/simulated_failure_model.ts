...
async function isModelManuallyDegraded(): Promise<boolean> {
  const degradedEndpoints =
    await ModelDegradationResource.listDegradedEndpoints();

  return degradedEndpoints.some(
    ({ modelId, providerId, host }) =>
      modelId === SIMULATED_FAILURE_MODEL_ENDPOINT.modelId &&
      providerId === SIMULATED_FAILURE_MODEL_ENDPOINT.providerId &&
      host === SIMULATED_FAILURE_MODEL_ENDPOINT.host
  );
}

/**
 * Both degradation stores are reported: routing skips a model flagged in either
 * one, so a status naming only the automatic store reads as healthy while the
 * streams are in fact routing around the model.
 */
export async function getSimulatedFailureModelStatus(): Promise<{
  failureEnabled: boolean;
  failureTriggered: boolean;
  automaticallyDegraded: boolean;
  ...
  const [
    state,
    automaticallyDegraded,
    manuallyDegraded,
    ttlSeconds,
    healthWindow,
  ] = await Promise.all([
    readFailureState(),
    isModelEndpointAutomaticallyDegraded(SIMULATED_FAILURE_MODEL_ENDPOINT),
    isModelManuallyDegraded(),
