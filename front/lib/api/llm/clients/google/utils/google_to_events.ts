...
export async function* streamLLMEvents({
  ...
  for await (const generateContentResponse of generateContentResponses) {
    accumulatedEvents.push(generateContentResponse);
    assert(
  ...
  const outputPath = path.join(__dirname, "google_events.json");
  fs.writeFileSync(outputPath, JSON.stringify(accumulatedEvents, null, 2));
}
  ...
  if (part.functionCall) {
    ...
    return {
      ...
      content: {
        id,
        name,
        // TODO: set arguments type as Record<string, unknown
