...
export type ModelCallConsumption<TCall, TResult> = {
  ...
  toolResultCreditsWereReduced: boolean;
};

type InputCreditAllocation = {
  inputCreditAmountMicro: number;
  resultCreditAmountsMicro: number[];
  toolResultCreditsWereReduced: boolean;
};
...
function allocateInputCredits({
  inputSideCreditAmountMicro,
  resultCreditAmountsMicro,
}: {
  inputSideCreditAmountMicro: number;
  resultCreditAmountsMicro: number[];
}): InputCreditAllocation {
  ...
  if (inputCreditAmountMicro >= 0) {
    return {
      inputCreditAmountMicro,
      resultCreditAmountsMicro,
      toolResultCreditsWereReduced: false,
    };
  }

  return {
    inputCreditAmountMicro: 0,
    resultCreditAmountsMicro: distributeByWeight({
      total: Math.max(inputSideCreditAmountMicro, 0),
      weights: resultCreditAmountsMicro,
    }),
    toolResultCreditsWereReduced: true,
  };
}

/**
 * @cc [owner:id13,label:backend;data-integrity] model-call-credit-conservation
 * When the exact provider cost covers the fixed output-side attribution, reconciled model and
 * tool amounts MUST sum to the provider's exact micro-credit cost.
 */
/**
 * @cc [owner:id13,label:backend;data-integrity] bounded-tool-result-attribution
 * When tool-result footprints exceed the remaining input-side credits, their reconciled amounts
 * MUST be reduced proportionally and the generic input amount MUST be zero, without changing their
 * gross attributed amounts.
 */
export function buildModelCallConsumption<TCall, TResult>({
  ...
  const inputAllocation = allocateInputCredits({
    inputSideCreditAmountMicro,
    resultCreditAmountsMicro,
  });
  ...
  return {
    input: {
      ...
      reconciledCreditAmountMicro: inputAllocation.inputCreditAmountMicro,
    },
    output: {
    ...
    consumedToolResults: consumedToolResults.map((result, index) => ({
      ...
      reconciledCreditAmountMicro:
        inputAllocation.resultCreditAmountsMicro[index],
    })),
    exactCreditAmountMicro,
    toolResultCreditsWereReduced: inputAllocation.toolResultCreditsWereReduced,
  };
}
