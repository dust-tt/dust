...
} from "@app/lib/api/assistant/consumption/model_call_allocation";
...
describe("buildModelCallConsumption", () => {
  ...
  it("debits a consumed tool result from the consuming call's input row", () => {
    ...
    expect(consumption.toolResultCreditsWereReduced).toBe(false);
  });
  ...
  it("clamps the input row to zero and scales the results to fit", () => {
    ...
    expect(consumption.toolResultCreditsWereReduced).toBe(true);
