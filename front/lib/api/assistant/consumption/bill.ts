...
export async function billExecution(
  ...
  const bill = await withTransaction(async (transaction) => {
    ...
    const { adjustmentItemIds, hasTrackableTool } = await settleToolCharges(
      auth,
      {
        actions,
    ...
    if (isFreeOrigin(triggeringUserMessageOrigin)) {
      const attributedItems = settledRows.filter(
        (row) =>
          row.itemType !== "tool_direct" &&
          row.itemType !== "tool_adjustment" &&
          row.reconciledCreditAmountMicro !== null &&
          row.reconciledCreditAmountMicro !== 0
      );
      await AgentMessageConsumptionItemResource.zeroReconciledCreditAmounts(
        auth,
        { items: attributedItems, transaction },
      );
      settledRows =
        await AgentMessageConsumptionItemResource.listConsumptionRowsByRunKey(
          auth,
