...
vi.mock("@app/temporal/credit_consumption/client", () => ({
  signalConsumptionEventsAppended: vi.fn(),
}));
...
describe("recordModelCallConsumption", () => {
  beforeEach(() => {
    ...
    vi.mocked(signalConsumptionEventsAppended).mockResolvedValue(
      new Ok(undefined)
    );
  ...
  it("records one model call once, however often the writer retries", async () => {
    ...
    expect(signalConsumptionEventsAppended).toHaveBeenCalledTimes(2);
    expect(signalConsumptionEventsAppended).toHaveBeenLastCalledWith(
      auth.toJSON(),
      { runKey: RUN_KEY }
    );
  });
