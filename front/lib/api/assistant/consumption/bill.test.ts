...
async function setupMessage({ origin }: { origin?: UserMessageOrigin } = {}) {
...
describe("billExecution", () => {
  ...
  it("carries the model-group rounding remainder on one row", async () => {
    ...
    const event = await AgentMessageConsumptionEventResource.fetchByEventKey(
      auth,
      { eventKey: `execution:${RUN_KEY_X}:billed` }
    );
    expect(event?.consumptionItemIds).toEqual(
      roundingRows.map((row) => row.id)
    );
  });
  ...
  it("zeros attributed credits but retains direct tool charges for a free origin", async () => {
    ...
    const attributedItems = firstItems.filter(
      (item) =>
