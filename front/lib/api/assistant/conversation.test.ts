...
describe("postUserMessage rate limiting", () => {
  let auth: Authenticator;
  let workspace: Awaited<ReturnType<typeof createResourceTest>>["workspace"];
  let conversation: ConversationType;
  let agentConfig: LightAgentConfigurationType;
  ...
  it("should not count human mentions toward the message limit", async () => {
    const mentionedUser = await UserFactory.basic();
    await MembershipFactory.associate(workspace, mentionedUser, {
      role: "user",
    });

    // Mix of human and agent mentions
    const mentions: MentionType[] = [
      {
        type: "user",
    ...
    const result = await postUserMessage(auth, {
      conversation,
      content: `Hello @${mentionedUser.username} and @${agentConfig.name}`,
      mentions,
      context: {
        username: userJson.username,
        timezone: "UTC",
    ...
    // Should only have 1 call for the agent mention, not 2 (one for each mention)
    expect(agentMentionRateLimiterCalls.length).toBe(1);
  });

  it("should count only agent mentions when multiple agents are mentioned", async () => {
    ...
    const result = await postUserMessage(auth, {
      conversation,
      content: `Hello @${agentConfig.name} and @${agentConfig2.name}`,
      mentions,
      context: {
        username: userJson.username,
        timezone: "UTC",
    ...
    // Should have 2 calls, one for each agent mention
    expect(agentMentionRateLimiterCalls.length).toBe(2);
  });

  it("should not call agent mention rate limiter when only human mentions are present", async () => {
    const mentionedUser1 = await UserFactory.basic();
    const mentionedUser2 = await UserFactory.basic();
    await MembershipFactory.associate(workspace, mentionedUser1, {
      role: "user",
    });
    await MembershipFactory.associate(workspace, mentionedUser2, {
      role: "user",
    });
    ...
    const result = await postUserMessage(auth, {
      conversation,
      content: `Hello @${mentionedUser1.username} and @${mentionedUser2.username}`,
      mentions,
      context: {
        username: userJson.username,
        timezone: "UTC",
    ...
    // Should have 0 calls since there are no agent mentions
    expect(agentMentionRateLimiterCalls.length).toBe(0);
  });
});
