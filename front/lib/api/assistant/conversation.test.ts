...
describe("postUserMessage", () => {
  ...
  describe("running agent guard", () => {
    it("should return the UI-aligned message when switching agents", async () => {
      ...
      expect(fetchedConversationResult.isOk()).toBe(true);
      if (fetchedConversationResult.isErr()) {
        return;
      }
      ...
      try {
        const result = await postUserMessage(auth, {
          conversation: fetchedConversationResult.value,
          content: `Hello @${agentConfig2.name}`,
          mentions: [
            {
        ...
        if (result.isErr()) {
          ...
          expect(result.error.api_error.message).toBe(
            `Wait for @${agentConfig1.name} to finish before switching agents`
          );
        }
        expect(launchAgentLoopWorkflow).not.toHaveBeenCalled();
      } finally {
