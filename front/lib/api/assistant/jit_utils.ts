...
export function listFiles(
  ...
  for (const versions of conversation.content) {
    ...
    const file = getFileForMessage(m);
    if (file) {
      files.push(file);
    }
  }
...
export function getFileForContentFragmentMessage(
  m: ContentFragmentType & {
    contentType: Exclude<
      SupportedContentFragmentType,
      SupportedImageContentType
    >;
...
export function getFileForMessage(
  m: MessageType
  ...
  if (
    ...
    return getFileForContentFragmentMessage(m);
    ...
    for (const f of generatedFiles) {
      ...
      const isExtractable = canDoJIT && isExtractableContentType(f.contentType);
