...
export async function duplicateAgentConfiguration(
  auth: Authenticator,
  agentConfigurationId: string
): Promise<AgentConfigurationType> {
  const user = auth.user();
  ...
  const dulpicatedDBAgentConfiguration = await front_sequelize.transaction(
    async (t) => {
      ...
      if (!agentConfiguration) {
        throw new Error(
          `Agent configuration ${agentConfigurationId} not found.`
        );
      }

      let newGenerationConfiguration: AgentGenerationConfiguration | null =
        null;
      if (agentConfiguration.generationConfigurationId) {
        newGenerationConfiguration = await AgentGenerationConfiguration.create(
          {
            prompt: agentConfiguration.generationConfiguration.prompt,
            providerId: agentConfiguration.generationConfiguration.providerId,
            modelId: agentConfiguration.generationConfiguration.modelId,
            temperature: agentConfiguration.generationConfiguration.temperature,
          },
          { transaction: t }
        );
      }

      let newRetrievalConfiguration: AgentRetrievalConfiguration | null = null;
      if (agentConfiguration.retrievalConfigurationId) {
        newRetrievalConfiguration = await AgentRetrievalConfiguration.create(
          {
            sId: generateModelSId(),
            query: agentConfiguration.retrievalConfiguration.query,
            queryTemplate:
              agentConfiguration.retrievalConfiguration.queryTemplate,
            relativeTimeFrame:
              agentConfiguration.retrievalConfiguration.relativeTimeFrame,
            relativeTimeFrameDuration:
              agentConfiguration.retrievalConfiguration
                .relativeTimeFrameDuration,
            relativeTimeFrameUnit:
              agentConfiguration.retrievalConfiguration.relativeTimeFrameUnit,
            topK: agentConfiguration.retrievalConfiguration.topK,
            topKMode: agentConfiguration.retrievalConfiguration.topKMode,
          },
          { transaction: t }
        );
      }

      let newAgentDustAppRunConfiguration: AgentDustAppRunConfiguration | null =
        null;
      if (agentConfiguration.dustAppRunConfigurationId) {
        newAgentDustAppRunConfiguration =
          await AgentDustAppRunConfiguration.create(
            {
              sId: generateModelSId(),
              appWorkspaceId:
                agentConfiguration.dustAppRunConfiguration.appWorkspaceId,
              appId: agentConfiguration.dustAppRunConfiguration.appId,
            },
            { transaction: t }
          );
      }
      let newAgentDatabaseQueryConfiguration: AgentDatabaseQueryConfiguration | null =
        null;
      if (agentConfiguration.databaseQueryConfigurationId) {
        newAgentDatabaseQueryConfiguration =
          await AgentDatabaseQueryConfiguration.create(
            {
              sId: generateModelSId(),
              dataSourceWorkspaceId:
                agentConfiguration.databaseQueryConfiguration
                  .dataSourceWorkspaceId,
              dataSourceId:
                agentConfiguration.databaseQueryConfiguration.dataSourceId,
              databaseId:
                agentConfiguration.databaseQueryConfiguration.databaseId,
            },
            { transaction: t }
          );
      }

      const newAgentSid = generateModelSId();
      const newAgentConfiguration = await AgentConfiguration.create(
        {
          sId: newAgentSid,
          version: agentConfiguration.version,
          status: agentConfiguration.status,
          scope: "workspace", // need to become private in the furue
          name: `${agentConfiguration.name}-${newAgentSid}`,
          description: agentConfiguration.description,
          pictureUrl: agentConfiguration.pictureUrl,
          workspaceId: agentConfiguration.workspaceId,
          generationConfigurationId: newGenerationConfiguration?.id,
          authorId: user.id,
          retrievalConfigurationId: newRetrievalConfiguration?.id,
          dustAppRunConfigurationId: newAgentDustAppRunConfiguration?.id,
          databaseQueryConfigurationId: newAgentDatabaseQueryConfiguration?.id,
        },
        { transaction: t }
      );
  ...
  const duplicatedAgentConfiguration = await getAgentConfiguration(
    auth,
    dulpicatedDBAgentConfiguration.sId
  );
  if (!duplicatedAgentConfiguration) {
    throw new Error(`Could not get the duplicated agent configuration`);
  }
  return duplicatedAgentConfiguration;
}
