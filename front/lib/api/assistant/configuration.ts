...
export async function updatePermissions(
  ...
  const users = await UserResource.fetchByIds(newUserIds);
  if (users.length !== newUserIds.length) {
    return new Err(new Error("Some user IDs were not found."));
  }

  // setMembers handles adding/removing users to match the provided list.
