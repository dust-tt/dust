...
export async function getSpaceCategoriesWithUsage(
  ...
  const [usages, webhookUsages, toolsUsage] = await Promise.all([
    ...
    getToolsUsage(auth, { withSkills: true }),
  ]);
  ...
  // Triggers aren't `DataSourceView`s either, so unlike actions below, this row's
  // count was never patched at all — it stayed at 0 and the row never rendered.
