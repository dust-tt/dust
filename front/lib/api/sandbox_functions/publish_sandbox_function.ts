...
/**
 * Publish a sandbox function: build the source the model wrote to the pod mount (the build
 * validates any declared databases' schema files), then store the bundle and its contract.
 * Publish never touches the live databases — schema changes reach them only through
 * `dsbx db reconcile`.
 *
