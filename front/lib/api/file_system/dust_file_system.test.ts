...
describe("DustFileSystem.resolveRenameDestination", () => {
  it("returns the sibling path for a new file name", () => {
    const result = DustFileSystem.resolveRenameDestination(
      "conversation-abc/reports/report.pdf",
      "summary.pdf"
    );

    assert(result.isOk());
    expect(result.value).toBe("conversation-abc/reports/summary.pdf");
  });

  it("returns the same path when the file name is unchanged", () => {
    const result = DustFileSystem.resolveRenameDestination(
      "conversation-abc/report.pdf",
      "report.pdf"
    );

    assert(result.isOk());
    expect(result.value).toBe("conversation-abc/report.pdf");
  });

  it.each([
    ["an empty filename", ""],
    ["a filename containing a forward slash", "sub/dir.pdf"],
    ["a filename containing a backslash", "sub\\dir.pdf"],
  ])("returns Err(invalid_path) for %s", (_label, newFileName) => {
    const result = DustFileSystem.resolveRenameDestination(
      "conversation-abc/report.pdf",
      newFileName
    );
