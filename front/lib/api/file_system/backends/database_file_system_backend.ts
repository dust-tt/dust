...
export class DatabaseFileSystemBackend implements FileSystemBackend {
  ...
  async read(
    ...
    try {
      ...
      const stream = await FileSystemContentResource.getReadStream(
        this.auth,
        this.scope,
        resolved.node.id
      );
  ...
  async write(
    ...
    try {
      ...
      const written = await FileSystemContentResource.writeContent(
        this.auth,
        this.scope,
        {
          nodeId: node.id,
  ...
  async getDownloadUrl(
    ...
    try {
      ...
      const result = await FileSystemContentResource.getDownloadUrl(
        this.auth,
        this.scope,
        resolved.node.id,
