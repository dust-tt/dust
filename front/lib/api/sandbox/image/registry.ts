...
SHELLEOF`,
  ...
  .registerTool({
    name: "pptx_inspect",
    description:
      "Inspect .pptx structure: slides, layouts, shapes, text, charts, tables, embedded media. Use before editing a deck to map layouts and shape positions. The per-slide view shows each text box's paragraph skeleton (slots p[i] with empty spacer paragraphs surfaced and per-paragraph colour) and lints with [!] blockers (overflow, edge bleed, distorted images where box ratio != native ratio, low-res images, stacked/coincident boxes) and [i] advisories (partial overlap with inches-to-separate, underfilled/off-centre text, tight margins, vertical anchor). --text tags each line with its shape #id and flags repeated template scaffolding. --qa N[,N,...] is the QA gate (run after a round of edits): it takes one slide or several (e.g. 2,5,7-9) and QA-ing them in one call shares a single render pass, so edit a batch of slides then QA them together; it prints each slide's #id-tagged text AND a boxed diagnostic render (each shape's bounding box labeled #id just outside it — a text box whose copy overflows is grown to wrap the rendered text, biased larger; overlap regions shaded red, including text spilling onto a neighbour; a note for decorative marker runs left with no text row beside them) — the core visual-QA signal for overflow/overlap/distortion/stranded-markers. --render rasterizes slide(s) to a plain JPEG (no overlay), a quick visual look only; the diagnostic boxes live in --qa. --compare FILE audits an edited deck against its source/template (orphaned slide parts, dropped media/fonts, stripped imagery, layout collapse, over-density, structural tally, content-slot fidelity — text written into the template's spacer paragraphs — and a shape-retention [i] advisory when a cloned slide kept under ~60% of its exemplar's shapes) and prints a [QA: PASS/FAIL] verdict — the authoritative structural gate before delivery",
    usage:
      "pptx_inspect <file> [--qa N[,N,...]] [--slide N] [--layouts] [--text] [--media] [--render] [--compare FILE] [--max-shapes N] [--offset N]",
    returns:
      "Deck overview, or one shape per line in slide view: '<id>  <kind>  <left,top WxH>  [ph=<type>]  <summary>' then 'p[i]: <text>' per paragraph (empty spacer slots shown), with [!] blockers and [i] advisories inline. --text tags lines with shape #id (for render readback) and flags repeated scaffolding. Layouts/media views emit format-specific listings. --qa N[,N,...] returns each slide's #id-tagged text plus a labeled bounding-box overlay (slide-NNN-boxes.png; #id outside each box; a text box whose copy overflows is grown to wrap the rendered text, biased larger) with a pixel-metrics digest of overlaps (incl. text spilling onto a neighbour) and unaligned marker runs; --render writes plain JPEGs (no overlay). --compare prints a template-fidelity audit ending in a [QA: PASS/FAIL] verdict",
    runtime: "system",
  })
