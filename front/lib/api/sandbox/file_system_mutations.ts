...
import {
  ...
  syncCanonicalFileResourceFromMount,
...
export const SandboxFileSystemMutationRequestSchema = z.discriminatedUnion(
  "operation",
  [
    MutationBaseSchema.extend({ operation: z.literal("mkdir") }).strict(),
    MutationBaseSchema.extend({ operation: z.literal("rmdir") }).strict(),
    ...
    MutationBaseSchema.extend({
      operation: z.literal("content_committed"),
    }).strict(),
  ]
);
...
function isIdempotentSuccess(error: DustFileSystemError): boolean {
  return error.code === "not_found" || error.code === "already_exists";
}
...
async function executeMutation(
  ...
  switch (request.operation) {
    case "mkdir": {
      const result = await dustFs.mkdir(sourcePath);
    ...
    case "rename": {
      ...
      if (!sourceExists.value && destinationExists.value) {
        ...
        return syncCanonicalFileResourceFromMount(
          auth,
          dustFs,
          destinationPath
        );
      ...
      if (statResult.value === null) {
        return new Err(
          new DustFileSystemError(
            "not_found",
            `Committed file not found: ${sourcePath}`
          )
        );
      }
      return syncCanonicalFileResourceFromMount(auth, dustFs, sourcePath);
    }
