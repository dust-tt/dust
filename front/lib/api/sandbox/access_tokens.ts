...
/**
 * Pins a sandbox tool execution to the JWT's specific (cId, mId) session.
 * Workspace match is enforced upstream by the auth wrapper; this additional
 * check prevents a sandbox client from polling or executing executions
 * belonging to a different session in the same workspace.
 *
 * Resolves the JWT's `mId` (a Message sId) to its AgentMessage model id and
 * compares against the execution's `agentMessageId`. Returns false on either
 * a missing message or a mismatch.
 */
export async function isSandboxToolExecutionInClaimsSession(
  auth: Authenticator,
  claims: SandboxExecTokenPayload,
  execution: SandboxToolExecutionType
): Promise<boolean> {
  ...
  return (
    ...
    execution.agentMessageId === message.agentMessageId
  );
}
