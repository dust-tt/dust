...
describe("consumption top rankings", () => {
  ...
  it("retrieves every group before ranking an unlimited request", async () => {
    const { auth } = await setup();
      ...
      .mockResolvedValueOnce(
        esResponse({
          by_group: {
            ...
            after_key: { value: "agent1" },
          },
          total_credit_micro: { value: 4_000_000 },
        })
      )
      .mockResolvedValueOnce(
        esResponse({
    ...
    expect(result.value.totalCredits).toBe(4);

    const [, firstOptions] = vi.mocked(searchConsumptionAnalytics).mock
      .calls[0];
    expect(firstOptions?.aggregations?.by_group?.composite).toMatchObject({
      size: 1_000,
      sources: [{ value: { terms: { field: "agent.id" } } }],
    });
    const [, secondOptions] = vi.mocked(searchConsumptionAnalytics).mock
      .calls[1];
    expect(secondOptions?.aggregations?.by_group?.composite).toMatchObject({
      after: { value: "agent1" },
    });
  });
