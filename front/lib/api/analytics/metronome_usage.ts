...
export class MetronomeUsageError extends Error {
  constructor(
    readonly type:
      | "metronome_not_configured"
      | "usage_fetch_failed"
      | "grouped_usage_invalid_group_key"
      | "grouped_usage_fetch_failed",
    readonly details: {
      cause?: string;
...
/**
 * Maps a metronome usage error to the standard `{ status_code, api_error }`
 * shape. Use this from any framework (Next or Hono) — only the response
 * dispatch differs.
 */
export function getMetronomeUsageApiError(
  err: MetronomeUsageError
  ...
  switch (err.type) {
    ...
    case "grouped_usage_invalid_group_key":
      return {
        status_code: 400,
        api_error: {
          type: "invalid_request_error",
          message:
            `Grouping by "${err.details.groupBy}" is not available. The billable metric ` +
            `must have "${err.details.eventProperty}" configured as a group key in Metronome.`,
        },
      };
    case "grouped_usage_fetch_failed":
      return {
        status_code: 500,
        api_error: {
          type: "internal_server_error",
          message: `Failed to retrieve Metronome grouped usage: ${err.details.cause ?? ""}`,
        },
      };
...
export async function getMetronomeUsage(
  ...
  if (!metronomeCustomerId) {
    return new Err(new MetronomeUsageError("metronome_not_configured"));
  }
  ...
  if (!groupBy) {
    ...
    if (result.isErr()) {
      return new Err(
        new MetronomeUsageError("usage_fetch_failed", {
    ...
    if (llmGroupedResult.isErr()) {
      ...
      if (isGroupKeyError) {
        return new Err(
          new MetronomeUsageError("grouped_usage_invalid_group_key", {
            groupBy,
            eventProperty,
          })
        );
      ...
      return new Err(
        new MetronomeUsageError("grouped_usage_fetch_failed", { cause: msg })
      );
    }
