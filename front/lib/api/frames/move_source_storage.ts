...
export class FrameSourceMoveStorageError extends Error {
  constructor(
    readonly code: "conflict" | "copy_failed" | "invalid_source",
    message: string
  ) {
    super(message);
    this.name = "FrameSourceMoveStorageError";
  }
}

function storageError(
  code: FrameSourceMoveStorageError["code"],
  message: string
) {
  return new Err(new FrameSourceMoveStorageError(code, message));
}
...
export async function inspectFrameSourceMoveStorage({
  ...
  Result<FrameSourceMoveStorageSnapshot, FrameSourceMoveStorageError>
> {
...
): Promise<Result<void, FrameSourceMoveStorageError>> {
  const bucket = getPrivateUploadBucket();
