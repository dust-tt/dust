 ...
 *
 * Lives here (not on `KeyResource`) because the flag read pulls in `lib/auth`,
 * which imports `KeyResource` — computing this inside the resource would be an
 * import cycle. The keys endpoints call this and pass the map to
 * `KeyResource.toJSONWithSpaces`.
 */
