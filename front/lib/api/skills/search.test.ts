...
describe("searchSkills pagination", () => {
  ...
  it("binds cursors to ranking and selection filters", async () => {
    ...
    for (const options of [
      ...
      { filters: { status: ["active" as const, "archived" as const] } },
    ]) {
      const changed = await searchSkills(auth, {
        searchTerm: "",
  ...
  it("searches active, archived, or both without including globals in archive-only pages", async () => {
    const { auth } = await createPrivateApiMockRequest({ role: "admin" });
    const active = await createDocument(auth, "AAA active");
    ...
    const defaults = await searchSkills(auth, { searchTerm: "" });
    assert(defaults.isOk());
    expect(
      defaults.value.skills.some((skill) => skill.sId === active.skill_id)
    ).toBe(true);
    expect(
      defaults.value.skills.every((skill) => skill.status === "active")
    ).toBe(true);

    const archives = await searchSkills(auth, {
      searchTerm: "",
      filters: { status: ["archived"] },
    });
    assert(archives.isOk());
    expect(archives.value.skills.map((skill) => skill.sId)).toEqual([
      archived.skill_id,
    ]);
    expect(
      mockSearch.mock.lastCall![0].query.bool.must[0].bool.filter
    ).toContainEqual({ terms: { status: ["archived"] } });

    const both = await searchSkills(auth, {
      searchTerm: "",
      filters: { status: ["archived", "active"] },
    });
    assert(both.isOk());
    expect(both.value.skills.map((skill) => skill.sId)).toEqual(
      expect.arrayContaining([active.skill_id, archived.skill_id, "go-deep"])
    );
    expect(
      mockSearch.mock.lastCall![0].query.bool.must[0].bool.filter
    ).toContainEqual({ terms: { status: ["active", "archived"] } });
  });

  it.each([
    "strict",
    "redact_unreadable",
  ] as const)("excludes a restored skill from the next archived page in %s mode", async (permissionFiltering) => {
    const { auth } = await createPrivateApiMockRequest({ role: "admin" });
    ...
    const page = await searchSkills(auth, options);
    assert(page.isOk() && page.value.nextCursor);
    expect(page.value.skills.map((skill) => skill.sId)).toEqual([first.sId]);
    await second.restore(auth);
    const next = await searchSkills(auth, {
      ...options,
      cursor: page.value.nextCursor,
    });
    assert(next.isOk());
    expect(next.value).toEqual({ skills: [], nextCursor: null });
  });

  it("applies default, availability, and editor filters to code-defined skills too", async () => {
    const { auth } = await createPrivateApiMockRequest({ role: "user" });
    serveDocuments([]);
    const defaults = await searchSkills(auth, {
      searchTerm: "",
      filters: { isDefault: true },
    });
    assert(defaults.isOk());
    ...
    const editors = await searchSkills(auth, {
      searchTerm: "",
      filters: { editedByMe: true },
    });
    assert(editors.isOk());
    expect(editors.value.skills).toEqual([]);
    const unpublished = await searchSkills(auth, {
      searchTerm: "",
  ...
  it("refetches custom hits displaced by globals without skips or duplicates", async () => {
    ...
    serveDocuments(documents);
    const first = await searchSkills(auth, {
      searchTerm: "",
    ...
    const second = await searchSkills(auth, {
      searchTerm: "",
      limit: globals.length + 2,
    ...
    const retry = await searchSkills(auth, {
      searchTerm: "",
      limit: globals.length + 2,
      cursor: first.value.nextCursor,
    });
  ...
  it("matches global aliases and does not return restricted globals", async () => {
    ...
    const restricted = await searchSkills(auth, {
  ...
  it("rejects a cursor for another query, workspace, caller, or missing state", async () => {
    ...
    for (const [caller, searchTerm, cursor] of [
      ...
      const result = await searchSkills(caller, {
        searchTerm,
        cursor,
      });
      assert(result.isErr());
  ...
  it("invalidates a strict cursor when pod read grants change", async () => {
    ...
    const restarted = await searchSkills(auth, {
      searchTerm: "",
      limit: 2,
    });
  ...
  it("uses committed metadata and excludes archived and cross-workspace hits in redaction mode", async () => {
    ...
    const [canonical] = await SkillResource.fetchByIds(auth, [
      current.skill_id,
    ]);
    assert(canonical);
    expect(custom).toEqual([canonical.toSearchJSON(auth, 1)]);
