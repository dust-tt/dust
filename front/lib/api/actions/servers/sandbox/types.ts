...
// `childActionId` is set by call_tool when a child sandbox tool execution is
// created. It is informational only — concurrent sandbox tool calls in one
// bash command will overwrite each other's `childActionId`. The source of
// truth for the set of blocked children is a scan over
// `sandbox_tool_executions` for the parent agent message (see
