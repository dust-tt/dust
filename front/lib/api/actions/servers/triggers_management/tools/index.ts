...
/**
 * Resolve which agent a triggers_management tool should act on.
 * Defaults to the agent in the current run. When targeting a different agent,
 * requires edit access (or workspace admin).
 */
async function resolveTargetAgent(
  auth: Authenticator,
  toolContext: ToolContextType | undefined,
  agentId: string | undefined
): Promise<Result<TargetAgent, MCPError>> {
  assert(
    isAgentLoopRunContext(toolContext?.runContext),
    "AgentLoopRunContext expected"
  );

  const currentAgent = toolContext.runContext.agentConfiguration;

  if (!agentId || agentId === currentAgent.sId) {
    return new Ok({ sId: currentAgent.sId, name: currentAgent.name });
  }
  ...
  if (!agent) {
    return new Err(
      new MCPError(
        `Agent not found or not accessible: ${agentId}. Pass a valid agent sId, or omit agentId to target the current agent.`,
        { tracked: false }
      )
    );
  }
  ...
  if (!agent.canEdit && !auth.isAdmin()) {
    return new Err(
      new MCPError(
        `You do not have permission to manage triggers for agent "${agent.name}" (${agent.sId}). You must be an editor of that agent.`,
        { tracked: false }
      )
    );
  }

  return new Ok({ sId: agent.sId, name: agent.name });
}
...
export function createTriggersManagementTools(
  ...
  const handlers: ToolHandlers<typeof TRIGGERS_MANAGEMENT_TOOLS_METADATA> = {
    create_schedule: async ({ name, schedule, prompt, timezone, agentId }) => {
      assert(
        isAgentLoopRunContext(toolContext?.runContext),
        "AgentLoopRunContext expected"
      );
      ...
      const targetAgentRes = await resolveTargetAgent(
        auth,
        toolContext,
        agentId
      );
      if (targetAgentRes.isErr()) {
        return new Err(targetAgentRes.error);
      }
      const targetAgent = targetAgentRes.value;
      ...
      try {
        result = await TriggerResource.makeNew(auth, {
          workspaceId: owner.id,
          agentConfigurationId: targetAgent.sId,
          name,
          kind: "schedule",
          status: "enabled",
      ...
      return new Ok([
        {
          type: "text" as const,
          text:
            `Created schedule "${name}" for agent "${targetAgent.name}"!\n\n` +
            `Schedule: ${schedule}\n` +
    ...
    list_triggers: async ({ agentId }) => {
      ...
      if (triggers.length === 0) {
        return new Ok([
          {
            type: "text" as const,
            text: `No triggers configured for agent "${targetAgent.name}".`,
          },
        ]);
      }
      ...
      // Batch-resolve webhook source names to avoid an N+1 per event trigger.
      const viewSIds = [
        ...new Set(
          webhookTriggers
      ...
      return new Ok([
        {
          type: "text" as const,
          text: `Triggers for agent "${targetAgent.name}":\n\n${sections.join("\n\n")}`,
        },
      ]);
    },

    disable_trigger: async ({ triggerId, agentId }) => {
      assert(
        isAgentLoopRunContext(toolContext?.runContext),
        "AgentLoopRunContext expected"
      );
      ...
      return new Ok([
        {
          type: "text" as const,
          text: `Disabled trigger "${triggerName}" on agent "${targetAgent.name}".`,
        },
      ]);
    },
    ...
    create_event_trigger: async ({
      ...
      // Resolve the requested source view, or return the catalog for discovery.
      ...
      // Validate the event against the source's subscribed events.
      ...
      // When a filter description is provided and the provider preset exposes a
      // payload schema for this event, generate and validate a filter expression.
      ...
      // Attach the trigger to a Pod when provided.
      ...
      getStatsDClient().increment(
        ...
        1,
        [`workspace_id:${owner.sId}`, `agent_id:${targetAgent.sId}`]
      );
      ...
      return new Ok([
        {
          type: "text" as const,
          text:
            `Created event trigger "${name}" for agent "${targetAgent.name}"!\n\n` +
            `Source: ${sourceName}\n` +
            `Event: ${event}\n` +
