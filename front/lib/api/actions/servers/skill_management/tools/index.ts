...
import { prewarmConversationSandbox } from "@app/lib/api/sandbox/prewarm";
...
const handlers: ToolHandlers<typeof SKILL_MANAGEMENT_TOOLS_METADATA> = {
  [ENABLE_SKILL_TOOL_NAME]: async ({ skillName }, { auth, runContext }) => {
    ...
    const definition = await GlobalSkillsRegistry.getById(auth, skill.sId);
    if (
      definition?.warmsConversationSandbox &&
      (await definition.warmsConversationSandbox(auth))
    ) {
