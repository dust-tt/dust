...
export const SANDBOX_FUNCTIONS_TOOLS_METADATA = createToolsRecord({
  ...
  publish: {
    description:
      ...
      "and its input and output JSON schemas are extracted from the `schema` export. Declared " +
      "pod databases' schema files are validated at build; publish never touches the live " +
      "databases. Re-publishing the same slug replaces the previous version.",
    schema: {
      slug: z
        .string()
