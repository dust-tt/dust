...
> {
  ...
  // Emit the per-member seat-change audit event as soon as the DB mutation is
  // committed — before reconciliation — so a committed change (including the
  // cancellation of a pending change) is always audited even if the Metronome
  // launch below fails. `seatChanged` covers cancellations, where the requested
  // and previous seat types match but a pending change was dropped.
  if (seatChanged) {
    void emitAuditLogEventDirect({
      workspace,
