...
// A nudge the user could have answered: one that opened a conversation.
// `replyAt` adds a message from the pod's user to that conversation.
async function createPostedNudge(
  auth: Authenticator,
  {
    activationPod,
    pod,
  ...
  if (!conversationResource) {
    throw new Error("Expected the nudge conversation to be fetchable.");
  }
...
describe("isEligibleForNudge", () => {
  ...
  it("is eligible again when the last nudge failed to post", async () => {
    const { authenticator, globalSpace } = await createResourceTest({
      role: "admin",
    });
    const activationPod = await createActivationPod(authenticator, globalSpace);
    const nudge = await ActivationNudgeFactory.create(authenticator, {
      activationPod,
      pod: globalSpace,
    });
    await nudge.markFailed("[no_seat] no seat");
  ...
  it("is not eligible once the max unanswered nudge count is reached, even outside the cap window", async () => {
    ...
    await createPostedNudge(refreshedAuth, {
      activationPod,
      pod: globalSpace,
  ...
  it("does not count nudges that never opened a conversation as unanswered", async () => {
    const { workspace, user, globalSpace } = await createResourceTest({
      role: "admin",
    });
    await WorkspaceResource.updateMetadata(workspace.id, {
  ...
  it("is eligible again once the user replies after the most recent nudge", async () => {
    ...
    // The user replied in the conversation the most recent nudge opened.
    await createPostedNudge(refreshedAuth, {
      activationPod,
      pod: globalSpace,
      createdAt: new Date(Date.now() - 5 * DAY_MS),
...
describe("postActivationNudge", () => {
  ...
  it("records the nudge against the pod and links its conversation", async () => {
    ...
    const nudge = await ActivationNudgeResource.fetchLatestSentForActivationPod(
      adminAuth,
      { activationPod }
    );
    if (!nudge) {
      throw new Error("Expected the nudge to be recorded.");
    }

    const [conversation] = await ConversationResource.fetchByModelIds(
      adminAuth,
      [nudge.conversationId ?? -1]
    );
  ...
  it("does not post to a pod the user is no longer a member of", async () => {
    ...
    expect(
      await ActivationNudgeResource.fetchLatestSentForActivationPod(adminAuth, {
        activationPod,
      })
    ).toBeNull();
  });
