...
export const toggleFeatureFlagPlugin = createPlugin({
  manifest: {
    ...
    description:
      "Toggle feature flags ON/OFF for this workspace. " +
      "Use the toggle below to switch between batch workspace-wide mode and group-scoped mode.",
    ...
    return new Ok({
      features: sortedFeatures.map((f) => {
        ...
        return {
          label: `[${FEATURE_FLAG_STAGE_LABELS[config.stage]}] ${f}`,
          value: f,
          checked: enabledFlagNames.has(f),
        };
      }),
