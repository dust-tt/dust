...
const VERSION_MAP_PATH = path.resolve(__dirname, "schema_versions.json");
...
describe("audit log schemas", () => {
  ...
  it("every AuditAction has a registered version in schema_versions.json", () => {
    // WorkOS validates each emitted event against the schema version we send
    // (see createAuditLogEvent). The map is generated by
    // `register_audit_log_schemas.ts --execute` and must be committed in the
    // same PR as the schema change — like a DB migration file. A missing entry
    // means the action would emit with no version once deployed.
    const versionMap = z
      .record(z.string(), z.number())
      .parse(JSON.parse(fs.readFileSync(VERSION_MAP_PATH, "utf-8")));

    const missing = AUDIT_ACTIONS.filter((action) => !(action in versionMap));
    expect(
      missing,
      `Missing schema versions for: ${missing.join(", ")}.\n` +
        `Run "npx tsx front/admin/register_audit_log_schemas.ts --execute" ` +
        `and commit front/lib/api/audit/schema_versions.json.`
    ).toEqual([]);
  });

  it("no orphan entries in schema_versions.json", () => {
    const versionMap = z
      .record(z.string(), z.number())
      .parse(JSON.parse(fs.readFileSync(VERSION_MAP_PATH, "utf-8")));

    const validActions = new Set<string>(AUDIT_ACTIONS);
    const orphans = Object.keys(versionMap).filter(
      (action) => !validActions.has(action)
    );
    expect(
      orphans,
      `Orphan entries in schema_versions.json (action not in AuditAction union): ${orphans.join(", ")}`
    ).toEqual([]);
  });
