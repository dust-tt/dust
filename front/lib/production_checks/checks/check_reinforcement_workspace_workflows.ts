...
import { makeWorkspaceScheduleId } from "@app/temporal/reinforcement/client";
...
async function getFlaggedWorkspaceIds(): Promise<string[]> {
  ...
  for (const workspace of allWorkspaces) {
    // Ignore workspaces created in the last 24h — the daily ensure-schedules
    // workflow hasn't had a chance to start their schedule yet.
...
export const checkReinforcementWorkspaceWorkflows: CheckFunction = async (
  ...
  for (const workspaceId of workspaceIds) {
    heartbeat();
    const workflowId = makeWorkspaceScheduleId(workspaceId);
