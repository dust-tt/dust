...
describe("listSpaceUnreadConversationsForUser", () => {
  ...
  it("should return lastUserActivityBySpace with the user's last read date", async () => {
    ...
    const activity = result.lastUserActivityBySpace.get(spaceModelIds[0]);
    expect(activity).toBeInstanceOf(Date);
    // Should be approximately 10 days ago (within a minute of test execution)
    const tenDaysAgo = dateFromDaysAgo(10);
    expect(Math.abs(activity!.getTime() - tenDaysAgo.getTime())).toBeLessThan(
      60_000
    );
  });

  it("should return the max lastReadAt across multiple conversations in the same space", async () => {
    ...
    // User read this one more recently (3 days ago vs 10 days ago for the first)
    await ConversationResource.upsertParticipation(userAuth, {
      conversation: recentConvo,
      action: "posted",
      user: userAuth.getNonNullableUser().toJSON(),
    ...
    const activity = result.lastUserActivityBySpace.get(spaceModelIds[0]);
    expect(activity).toBeInstanceOf(Date);
    // Should reflect the more recent read (3 days ago), not the older one (10 days ago)
    const threeDaysAgo = dateFromDaysAgo(3);
    expect(Math.abs(activity!.getTime() - threeDaysAgo.getTime())).toBeLessThan(
      60_000
    );
  ...
  it("should return an empty map when the user has no read activity in the spaces", async () => {
    // Create a new user with no participation in any space conversation
    const freshUser = await UserFactory.basic();
    await MembershipFactory.associate(workspace, freshUser, { role: "user" });
    ...
    expect(result.lastUserActivityBySpace.size).toBe(0);
  });
});
