...
describe("upsertMCPServerViews", () => {
  it("should create and update views without an authenticated user", async () => {
    ...
    const userlessAuthenticator = await Authenticator.internalAdminForWorkspace(
      workspace.sId
    );

    const createResult = await ConversationResource.upsertMCPServerViews(
      userlessAuthenticator,
      {
        conversation,
    ...
    let [result] = await ConversationResource.fetchMCPServerViews(
      userlessAuthenticator,
      conversation
    );
    expect(result).toMatchObject({ userId: null, enabled: true });

    const updateResult = await ConversationResource.upsertMCPServerViews(
      userlessAuthenticator,
      {
        conversation,
        mcpServerViews: [mcpServerView],
    ...
    [result] = await ConversationResource.fetchMCPServerViews(
      userlessAuthenticator,
      conversation
    );
    expect(result).toMatchObject({ userId: null, enabled: false });
  });
});
