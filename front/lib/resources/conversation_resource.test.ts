...
describe("listConversationsForAgents", () => {
  let auth: Authenticator;
  let workspace: LightWorkspaceType;
  let agentA: LightAgentConfigurationType;
  let agentB: LightAgentConfigurationType;

  beforeEach(async () => {
    const testSetup = await createResourceTest({ role: "builder" });
    workspace = testSetup.workspace;
    auth = testSetup.authenticator;
    agentA = await AgentConfigurationFactory.createTestAgent(auth, {
      name: "Agent A listConversationsForAgents",
    });
    agentB = await AgentConfigurationFactory.createTestAgent(auth, {
      name: "Agent B listConversationsForAgents",
    });
  });

  it("returns empty array when agentSIds is empty", async () => {
    const result = await ConversationResource.listConversationsForAgents(auth, {
      agentSIds: [],
      cutoffDate: dateFromDaysAgo(100),
    });
    expect(result).toEqual([]);
  });

  it("returns agent sIds whose conversations were updated on or after cutoffDate", async () => {
    const recent = dateFromDaysAgo(1);
    const conv = await ConversationFactory.create(auth, {
      agentConfigurationId: agentA.sId,
      messagesCreatedAt: [recent],
    });
    await ConversationFactory.setUpdatedAtForTest(auth, conv.id, recent);

    const result = await ConversationResource.listConversationsForAgents(auth, {
      agentSIds: [agentA.sId, agentB.sId],
      cutoffDate: dateFromDaysAgo(5),
    });
    expect(result).toContain(agentA.sId);
    expect(result).not.toContain(agentB.sId);
  });

  it("excludes agents when conversation updatedAt is before cutoffDate", async () => {
    const conv = await ConversationFactory.create(auth, {
      agentConfigurationId: agentA.sId,
    ...
    const result = await ConversationResource.listConversationsForAgents(auth, {
      agentSIds: [agentA.sId],
      cutoffDate: dateFromDaysAgo(5),
    });
    expect(result).not.toContain(agentA.sId);
  });

  it("with excludeHumanOutOfTheLoop still includes conversations that have a user message", async () => {
    const recent = dateFromDaysAgo(1);
    const conv = await ConversationFactory.create(auth, {
      agentConfigurationId: agentA.sId,
      messagesCreatedAt: [recent],
  ...
  it("with excludeHumanOutOfTheLoop excludes agent-only conversations without a trigger", async () => {
    const conv = await ConversationFactory.create(auth, {
      agentConfigurationId: agentA.sId,
      messagesCreatedAt: [],
      visibility: "unlisted",
    ...
    const withFlag = await ConversationResource.listConversationsForAgents(
      auth,
      {
        agentSIds: [agentA.sId],
        cutoffDate: dateFromDaysAgo(5),
    ...
    expect(withFlag).not.toContain(agentA.sId);

    const withoutFlag = await ConversationResource.listConversationsForAgents(
      auth,
      {
        agentSIds: [agentA.sId],
        cutoffDate: dateFromDaysAgo(5),
    ...
    expect(withoutFlag).toContain(agentA.sId);
  });

  it("with excludeHumanOutOfTheLoop includes triggered agent-only conversations", async () => {
