...
describe("GroupResource", () => {
  ...
  describe("listGroupsForUserModelIdsInWindow", () => {
    ...
    const WINDOW = { start: FEBRUARY, end: MARCH };
    ...
    it("includes a membership that started before the window and is still ongoing", async () => {
      ...
      const result = await GroupResource.listGroupsForUserModelIdsInWindow({
        workspace,
        userModelIds: [member.id],
        window: WINDOW,
      });

      expect(result.get(member.id)?.map((g) => g.name)).toEqual(["Sales"]);
    });

    it("includes a membership that ended inside the window", async () => {
      const member = await UserFactory.basic();
      await MembershipFactory.associate(workspace, member, { role: "user" });
      const sales = await GroupResource.makeNew({
        name: "Sales",
      ...
      // Left the group right at the window's start — still overlaps it.
      await setGroupMembershipWindow(sales, member, {
        startAt: JANUARY,
        endAt: FEBRUARY,
      });
    ...
    it("excludes a membership that ended before the window started", async () => {
      const member = await UserFactory.basic();
      await MembershipFactory.associate(workspace, member, { role: "user" });
      const sales = await GroupResource.makeNew({
        name: "Sales",
    ...
    it("excludes a membership that had not started yet by the window's end", async () => {
      const member = await UserFactory.basic();
      await MembershipFactory.associate(workspace, member, { role: "user" });
      const sales = await GroupResource.makeNew({
        name: "Sales",
