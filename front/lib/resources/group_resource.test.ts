...
describe("GroupResource", () => {
  ...
  describe("seat-granting groups", () => {
    ...
    describe("computeUserSeatFromGroups", () => {
      ...
      it("breaks a same-tier cadence tie toward the annual variant", async () => {
        const proGroup = await makeSeatGroup("g-pro", "pro");
        const proYearlyGroup = await makeSeatGroup(
          "g-pro-yearly",
          "pro_yearly"
        );
        await proGroup.dangerouslyAddMembers(authenticator, {
        ...
        await proYearlyGroup.dangerouslyAddMembers(authenticator, {
          users: [member.toJSON()],
          allowProvisionedGroups: true,
        });
