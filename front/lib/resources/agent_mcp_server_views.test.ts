...
describe("destroyAgentMCPServerConfigurationsForDataSourceView", () => {
  it("removes the tools linked to a hard-deleted data source view and refreshes the affected agents", async () => {
    ...
    const linkTo = (
      view: typeof deletedView,
      mcpServerConfigurationId: number
    ) =>
      AgentDataSourceConfigurationModel.create({
    ...
    await linkTo(deletedView, tool.id);
    await linkTo(keptView, tool.id);
    const otherLink = await linkTo(keptView, otherTool.id);
