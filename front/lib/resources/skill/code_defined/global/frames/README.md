...
Pass a Frame folder containing `manifest.json` or `index.tsx`. The script fetches the Viz
declarations, writes standard project configs and runs Oxlint directly on the source folder.
...
The generated `tsconfig.json` and `.oxlintrc.json` are refreshed on later runs. Existing configs
without the `Generated by Frame lint` marker are left untouched and the command fails.
