...
xlsx_inspect /files/conversation/budget.xlsx --grep '='                   # all formulas
xlsx_inspect /files/conversation/budget.xlsx --grep 'fill: FFFFFF00' --meta  # cells by metadata
...
format, font color, and background fill color. Use it to map: which cells are inputs (blue, no
...
Run \`xlsx_inspect <file> --grep '='\` on the output. Confirm every
...
- **Coloring: respect the spreadsheet's existing convention.** When
  ...
  (e.g. \`font: FF0000FF\`) and solid fill ARGB
  (e.g. \`fill: FFFFFF00\` for a yellow highlight) on each formatted
  cell. Theme and indexed colors are resolved to their displayed ARGB,
  so you never need to drop into Python to read them. Match the colors
  already in use for inputs, formulas, links, and highlighted
  assumption rows — \`--grep 'fill: <ARGB>' --meta\` is the fastest
  way to find every cell that shares a highlight. Only introduce a new
