...
export class GroupResource extends BaseResource<GroupModel> {
  ...
  /**
   * For each user, the groups whose membership overlapped `window` at all
   * (started before the window ended, and either never ended or ended after
   * the window started) — not just membership as of `now`. Used to resolve
   * "who was in which group" for a historical period, e.g. consumption
   * analytics filters scoped to a past billing cycle. Single batched query
   * (membership rows, then the groups they reference), no N+1.
   */
  static async listGroupsForUserModelIdsInWindow({
    workspace,
    userModelIds,
    groupKinds = ["regular_auto", "provisioned"],
    window,
    ...
    const memberships = await GroupMembershipModel.findAll({
      where: {
        ...
        userId: userModelIds,
        status: "active",
        startAt: { [Op.lte]: window.end },
        [Op.or]: [{ endAt: null }, { endAt: { [Op.gte]: window.start } }],
      },
    });
    if (memberships.length === 0) {
      return result;
    ...
    const groupById = new Map(
      groups.map((group) => [group.id, new this(GroupModel, group.get())])
    );
