...
export class ProjectTodoResource extends BaseResource<ProjectTodoModel> {
  ...
  static async fetchByItemIds(
    ...
    Map<{ itemId: string; userModelId: number }, ProjectTodoResource>
  > {
    if (itemIds.length === 0) {
      return new Map();
    }
    ...
    const result = new Map<
      { itemId: string; userModelId: number },
      ProjectTodoResource
    >();
    for (const source of sources) {
      ...
      result.set({ itemId: source.itemId, userModelId: source.userId }, todo);
    }
