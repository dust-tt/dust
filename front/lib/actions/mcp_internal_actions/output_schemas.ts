...
export const ToolGeneratedFileSchema = z.object({
  text: z.string(),
  uri: z.string(),
...
export function validateResourceOutput<
  T extends (typeof EmbeddedResourceSchemas)[number],
>(
  outputBlock: MCPToolResultContentType,
  schema: T
): outputBlock is { type: "resource"; resource: z.infer<T> } {
  return (
    outputBlock.type === "resource" &&
    schema.safeParse(outputBlock.resource).success
  );
}

const EmbeddedResourceSchemas = [
...
const MCPToolResultContentSchema = z.union([
  ...
  z.object({
    ...
    resource: z.union(EmbeddedResourceSchemas),
  }),
]);
