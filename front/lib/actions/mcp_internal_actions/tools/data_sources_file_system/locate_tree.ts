...
export function registerLocateTreeTool(
  ...
  server.tool(
    ...
    FILESYSTEM_LOCATE_IN_TREE_TOOL_DESCRIPTION,
    ...
    withToolLogging(
      ...
      async (params) => locateTreeToolCallback(auth, params)
    )
  );
}

export function registerLocateTreeToolWithDynamicTags(
  auth: Authenticator,
  server: McpServer,
  agentLoopContext: AgentLoopContextType | undefined
) {
  server.tool(
    ...
    withToolLogging(
      ...
      async (params) =>
        locateTreeToolCallback(auth, params, {
...
async function locateTreeToolCallback(
  auth: Authenticator,
