...
// Configurable versions for advanced_web_tools server

interface WebSearchToolConfig {
  providerSchema: ZodTypeAny;
}

export function registerWebSearchToolWithConfig(
  auth: Authenticator,
  server: McpServer,
  agentLoopContext: AgentLoopContextType | undefined,
  config: WebSearchToolConfig
) {
  const schema = WebsearchInputSchema.extend({
    provider: config.providerSchema,
  });
  ...
  server.tool(
    ...
    withToolLogging(
      ...
      async ({ query, provider }) => {
        ...
        // Use the configured provider (defaults to firecrawl)
        // For now, only firecrawl is supported
        const _configuredProvider = provider?.value ?? "firecrawl";
...
interface WebBrowserToolConfig {
  summarizationSchema: ZodTypeAny;
}

export function registerWebBrowserToolWithConfig(
  auth: Authenticator,
  server: McpServer,
  agentLoopContext: AgentLoopContextType | undefined,
  config: WebBrowserToolConfig
) {
  // Create schema with configurable summarization
  const schema = z.object({
    ...
    useSummary: config.summarizationSchema,
  });
