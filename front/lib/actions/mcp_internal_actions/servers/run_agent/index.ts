            ...
            params: {
              ...
              data: {
                ...
                output: {
                  ...
                  childAgentId: childAgentId,
=======
      // If the child conversation already finished while we were disconnected,
      // return success immediately using the stored agent message/output.
      {
        ...
        if (latestMessages.length > 0) {
          ...
          if (agentMsg.status === "succeeded") {
            let finalContent: string = agentMsg.content ?? "";
            let chainOfThought: string = agentMsg.chainOfThought ?? "";
              ...
              content: [
                {
                  ...
                  resource: {
                    ...
                    uri: convoUrl,
                    refs:
                      Object.keys(newRefs).length > 0 ? newRefs : undefined,
                  },
                },
                ...files.map((file) => ({
...
>>>>>>> Stashed changes
        ...
        // Early-finish detection: if the child conversation already completed successfully
        // (e.g., during a disconnect), return success immediately using stored output.
        {
          const latestMessages = conversation.content
            .map((versions) => versions[versions.length - 1])
            .filter(
        ...
        try {
          ...
          // Fallback: on stream termination, re-fetch conversation and return success
          // if the child agent has completed successfully.
          const refreshed = await api.getConversation({
            conversationId: conversation.sId,
          });
          if (refreshed.isOk()) {
            const conv2 = refreshed.value;
            const latestMessages2 = conv2.content
              .map((versions) => versions[versions.length - 1])
              .filter(
                (m: any) =>
            ...
            if (latestMessages2.length > 0) {
              const agentMsg2: any = latestMessages2[0];
              if (agentMsg2.status === "succeeded") {
                let finalContent: string = agentMsg2.content ?? "";
                let chainOfThought: string = agentMsg2.chainOfThought ?? "";

                let refsFromAgent: Record<string, CitationType> = {};
                let files: ActionGeneratedFileType[] = [];
                if (isMCPActionArray(agentMsg2.actions)) {
                  refsFromAgent = getCitationsFromActions(agentMsg2.actions);
                  files = agentMsg2.actions.flatMap((action: any) =>
                    action.generatedFiles.filter((f: any) => !f.hidden)
                  );
                }

                const convoUrl = `${config.getClientFacingUrl()}/w/${auth.getNonNullableWorkspace().sId}/assistant/${conv2.sId}`;
                ...
                return new Ok([
                  {
                    ...
                    resource: {
                      mimeType:
                        INTERNAL_MIME_TYPES.TOOL_OUTPUT.RUN_AGENT_RESULT,
...
=======
        ...
        // As a fallback, re-fetch the conversation and check if it completed successfully
        // while the stream was terminating.
        const refreshed = await api.getConversation({
          conversationId: conversation.sId,
        });
        if (refreshed.isOk()) {
