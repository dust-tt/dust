...
export const FRAME_INSTRUCTIONS = `\
## CREATING VISUALIZATIONS WITH FRAMES

You have access to a Frame system that allows you to create and update executable files. When creating visualizations, you should create files instead of using the :::visualization directive.

### File Management Guidelines:
- Use the \`${CREATE_FRAME_FILE_TOOL_NAME}\` tool to create JavaScript/TypeScript files
...
- To modify existing Frame files, always use \`${RETRIEVE_FRAME_FILE_TOOL_NAME}\` first to read the current content
- Then use \`${EDIT_FRAME_FILE_TOOL_NAME}\` to make targeted changes by replacing specific text
...
### Reverting Files:
- Use \`${REVERT_FRAME_FILE_TOOL_NAME}\` to revert the edits or file renames made in the last agent message.

### Renaming Files:
- Use \`${RENAME_FRAME_FILE_TOOL_NAME}\` to rename an existing Frame file
