...
/**
 * Builds a progress notification for frame file operations
 */
function buildFrameFileNotification(
...
/**
 * Frame Server - Allows the model to create and update frame files.
 * Frame includes any file that users can execute, run, or interact with directly.
...
const createServer = (
  ...
  const server = makeInternalMCPServer("frame");
  ...
  server.tool(
      ...
      "**Critical:** Multiple matches or inexact matches will cause failure.",
    {
      file_id: z
        .string()
        .describe(
          "The ID of the Frame file to update (e.g., 'fil_abc123')"
        ),
      old_string: z
        .string()
        .describe(
  ...
  server.tool(
    ...
    "Reverts a Frame file by canceling the edits and file renames in the last agent message.",
    {
      file_id: z
        .string()
        .describe(
          "The ID of the Frame file to revert (e.g., 'fil_abc123')"
        ),
    },
    withToolLogging(
      auth,
  ...
  server.tool(
    ...
    "Rename a Frame file. Use this to change the file name while keeping the content unchanged.",
    {
      file_id: z
        .string()
        .describe(
          "The ID of the Frame file to rename (e.g., 'fil_abc123')"
        ),
      new_file_name: z
        .string()
        .describe(
  ...
  server.tool(
      ...
      "understand the current file state and identify the exact text to replace.",
    {
      file_id: z
        .string()
        .describe(
          "The ID of the Frame file to retrieve (e.g., 'fil_abc123')"
        ),
    },
    withToolLogging(
      auth,
