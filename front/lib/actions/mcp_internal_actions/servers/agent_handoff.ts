...
const serverInfo: InternalMCPServerDefinitionType = {
  name: "agent_handoff",
  version: "1.0.0",
  description: "Rely on a sub-agent to defer certain queries.",
  icon: "robot",
  authorization: null,
};
...
function createServer(
  ...
  server.tool(
    ...
    "Defer a query to another agent that is more specialized and more fit to answer the user query.",
    {
    ...
    async ({ query, childAgent: { uri } }) => {
      if (!conversation) {
        return makeMCPToolTextError(
          "Unreachable: calling agent_handoff tool without a conversation."
        );
      }
      if (!getAgentConfiguration) {
        return makeMCPToolTextError(
          "Unreachable: calling agent_handoff tool without a getAgentConfiguration callback."
        );
      }
