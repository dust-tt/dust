...
/**
 * Represents an unsaved MCP server configuration with dataSources instead of dataSourceConfigurations.
 * This is used during the configuration process before the configuration is saved to the database.
 */
 ...
 */
type NonMCPActionConfigurationType = Exclude<
  AgentActionConfigurationType["type"],
  "mcp_server_configuration"
>;

/**
 * Type for unsaved agent action configurations.
 * Special handling for MCP server configuration to use UnsavedMCPServerConfigurationType.
 * The Exclude never is used to make sure the union type is correctly distributed.
 * TODO(mcp): clean this up following the migration of existing actions to MCP.
 */
export type UnsavedAgentActionConfigurationType =
  | Exclude<
      {
        [K in NonMCPActionConfigurationType]: UnsavedConfiguration<
          Extract<AgentActionConfigurationType, { type: K }>
        >;
      }[NonMCPActionConfigurationType],
      never
    >
