...
function toAnthropicContent(input: Content[]): (TextBlockParam | ImageBlockParam)[] {
...
function assistantContentToMessageParam(content: string | Content[]) : MessageParam {
    if (typeof content === "string") {
        return {
...
function toolResultContentToMessageParam(tool_use_id: string, content: string | Content[]) : MessageParam {
    if (typeof content === "string") {
        return {
            role: "assistant" as const,
            content: [
    ...
    for (const message of conversation.messages) {
        ...
        switch (message.role) {
            ...
            case "assistant":
                ...
                break;
            case "function":
                messages.push(toolResultContentToMessageParam(message.function_call_id, message.content));
                break;
            default:
                continue;
        }
