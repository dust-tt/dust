...
makeScript({}, async ({ execute }, logger) => {
  ...
  if (statusColumnExists.count > 0) {
    logger.info(
      "Status column already exists, checking if migration is needed"
    );

    // Check if isActive column still exists (needs cleanup)
    const [isActiveColumnExists] = await frontSequelize.query<{
      count: number;
    }>(
      `
      SELECT COUNT(*) as count
  ...
  if (!execute) {
    ...
    logger.info("3. Drop isActive column");
    return;
  }
