...
type ConsumptionCreditsFilterAggregate = CreditsFilterAggregate & {
  by_status?: StatusCreditsAggregate;
  ...
  async ({ workspaceId }, logger) => {
    ...
    const comparisons = activeUsers.map(({ seatType, userId }) => {
      ...
      const consumptionSelected =
        seatType === "free" ? consumption?.free : consumption?.paid;
      const consumptionMicroCredits = consumptionSelected?.creditMicro ?? 0;
      ...
      return {
        ...
        consumption: {
          ...
          paidAwuCredits: microCreditsToCredits(
            consumption?.paid.creditMicro ?? 0
          ),
          freeAwuCredits: microCreditsToCredits(
            consumption?.free.creditMicro ?? 0
          ),
          awuCreditsByStatus: consumptionSelected?.awuCreditsByStatus ?? {},
          awuCreditsByUsageType:
            consumptionSelected?.awuCreditsByUsageType ?? {},
        },
        metronome: {
    ...
    const consumptionSeatSplitUsers = comparisons.filter(
      ({ consumption }) =>
        consumption.paidAwuCredits !== 0 && consumption.freeAwuCredits !== 0
    );
    ...
    const summary = {
      ...
      consumptionSeatSplitUserCount: consumptionSeatSplitUsers.length,
      legacyConsumerAwuCredits,
      consumptionConsumerAwuCredits,
      metronomeConsumerAwuCredits,
