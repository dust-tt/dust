...
CREATE TABLE IF NOT EXISTS "agent_github_get_pull_request_actions" ("createdAt" TIMESTAMP WITH TIME ZONE NOT NULL, "updatedAt" TIMESTAMP WITH TIME ZONE NOT NULL, "owner" VARCHAR(255) NOT NULL, "repo" VARCHAR(255) NOT NULL, "pullNumber" INTEGER NOT NULL, "pullBody" TEXT, "pullCommits" JSONB[],"pullDiff" TEXT, "functionCallId" VARCHAR(255), "functionCallName" VARCHAR(255), "step" INTEGER NOT NULL, "id"  BIGSERIAL , "agentMessageId" BIGINT NOT NULL REFERENCES "agent_messages" ("id") ON DELETE NO ACTION ON UPDATE CASCADE, PRIMARY KEY ("id"));
