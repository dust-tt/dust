...
} from "@app/temporal/consumption/activities";
...
vi.mock("@app/lib/api/assistant/consumption/counters", () => ({
  applyExecutionTotal: mocks.applyExecutionTotal,
  readExecutionTotal: vi.fn(),
  recordExecutionCreditCounters: vi.fn(),
}));
...
describe("applyConsumptionEventsActivity", () => {
  ...
  it("upserts one Elasticsearch snapshot before acknowledging a message batch", async () => {
    ...
    expect(mocks.applyExecutionTotal).toHaveBeenCalledWith({
      workspaceId: "workspace",
      runKey: "execution-x",
      rootAgentMessageId: 101,
      agentMessageId: 7,
      totalCreditAmountMicro: 123,
    });
  ...
  it("quarantines events when one run key changes message identity", async () => {
    ...
    expect(mocks.applyExecutionTotal).not.toHaveBeenCalled();
