...
import { LabsConnectionsConfiguration } from "@app/lib/resources/labs_connections_resource";
...
export async function syncLabsConnection(
  configurationId: string
): Promise<void> {
  const configuration =
    await LabsConnectionsConfiguration.getById(configurationId);
  if (!configuration) {
  ...
  if (!configuration.dataSourceId) {
    throw new Error(
      `Configuration ${configurationId} has no data source configured`
    );
  }
