import type * as activities from "@app/temporal/project_task/activities";
import {
  condition,
  continueAsNew,
  proxyActivities,
...
const { analyzeProjectTodosActivity, mergeTasksForProjectActivity } =
...
export async function projectTodoWorkflow({
  ...
  while (true) {
    ...
    await mergeTasksForProjectActivity({ workspaceId, spaceId, runId });
