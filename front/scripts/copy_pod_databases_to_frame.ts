...
/**
 * Whether `relativePath` sits under a `{databaseName}.db/` replica directory whose name is a legal
 * database name. The path is copied verbatim, so this only decides what belongs in the plan.
 */
function isPodDatabaseReplicaRelativePath(relativePath: string): boolean {
  ...
  return POD_DATABASE_NAME_REGEX.test(
    databaseDirectory.slice(0, -".db".length)
  );
}

/**
 * @cc [owner:davidebbo,label:product] preserve-pod-database-name
 * A Pod replica directory keeps its name in Frame state: `{databaseName}.db` maps to
 * `{databaseName}.db`.
 */
export function planPodDatabaseCopy({
  ...
  const copies = [...sourceObjectPaths].sort().map((sourcePath) => {
    ...
    if (!isPodDatabaseReplicaRelativePath(relativePath)) {
      throw new Error(
