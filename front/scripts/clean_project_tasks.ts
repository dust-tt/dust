  ...
  async ({ execute, wId, pId, userId, butlerOnly }, logger) => {
    ...
    await frontSequelize.transaction(async (transaction) => {
      ...
      if (todoIds.length > 0) {
        ...
        const versionDeleted = await ProjectTaskVersionModel.destroy({
          where: todoWhere,
          transaction,
        });
        const conversationDeleted = await ProjectTaskConversationModel.destroy({
          where: todoChildWhere,
          transaction,
        });
        const sourceDeleted = await ProjectTaskSourceModel.destroy({
          where: todoChildWhere,
          transaction,
        });
        const todoDeleted = await ProjectTaskModel.destroy({
          where: todoWhere,
          transaction,
        });
      ...
      if (!butlerOnly) {
        const stateDeleted = await ProjectTaskStateModel.destroy({
          where: {
            workspaceId,
            spaceId: spaceId,
            ...(userModelId !== null ? { userId: userModelId } : {}),
