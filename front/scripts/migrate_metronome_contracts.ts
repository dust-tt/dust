...
async function migrateWorkspace(
  workspace: { sId: string; id: number; name: string },
  execute: boolean,
  logger: Logger,
  packageInfo: {
    aliasToPackageId: Record<string, string>;
  ...
  if (activeContracts.length === 0) {
    ...
    const newContract = await client.v1.contracts.create({
      ...
      uniqueness_key: `provision-${workspace.sId}-${subInfo.packageAlias}-${Date.now()}`,
    });
  ...
  for (const contract of activeContracts) {
    ...
    const newContract = await client.v1.contracts.create({
      ...
      uniqueness_key: `migrate-${workspace.sId}-${targetAlias}-${Date.now()}`,
    });
