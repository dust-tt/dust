...
const simulatedFailureModel = async (
  ...
  switch (command) {
    case "enable": {
      ...
      if (status.automaticallyDegraded) {
        throw new Error(
          "The simulated failure model is still in automatic recovery; wait for recovery before starting another run."
        );
      }
      // The streams already route around a manually flagged model, so a run
      // would never reach the synthetic endpoint to fail in the first place.
      if (status.manuallyDegraded) {
        throw new Error(
