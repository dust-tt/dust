...
| Enterprise custom plan  | Custom | Defined in stripe subscription |
...
- **Basic messages = 0 credits** (no premium features, no advanced model → unlimited with a peat)
...
> **Question:** Can free seats also send unlimited basic messages, or only paid seats ?
> 
...
| --- | --- | --- | --- | --- |
| **Free** | $0 | $0 | 300 | Auto — default for all new members joining a workspace  |
| **Pro** | $24/seat | $30/seat | 5,000 | Upgrade — admin-initiated (or auto when Free credits exhausted, if decided)  |
| **Max** | $100/seat | $125/seat | 20,000 | Upgrade — admin-initiated only |
...
> **UNRESOLVED — Seat/access model is evolving.** The table above reflects the Mar 27 proposal. The Mar 30 Slack discussion introduced a more developed model that diverges significantly:
> 
> 
> 
> | Access level | Type | Credits | Notes |
> | --- | --- | --- | --- |
> | **Viewer** | Not a seat — conversation-level sharing | ~20 AWU/conversation or 300 lifetime | For people outside the workspace viewing shared conversations |
> | **Admin** | Seat (free, 0 credits) | 0 | Manages connections, workspace settings |
> | **Light** | Seat (Enterprise Platform plan only) | From workspace pool | Limited actions, no builder/admin access |
> | **Free** | Seat | 300/month (or 300 lifetime?) | Default for new members on self-serve |
> | **Pro** | Seat | 5,000/month | Upgrade |
> | **Max** | Seat | 20,000/month | Upgrade |
> 
> Key differences from the table above: (1) Viewer is not a seat at all, it's conversation-level access with a lifetime cap, not monthly; (2) Admin seat exists at $0 with no credits; (3) Light seat is Enterprise-only with workspace pool credits; (4) Free seat may be 300 lifetime, not 300/month — fundamentally different allocation model.
> 
> **This needs reconciliation before implementation.** The Metronome package configuration (seat products, recurring credits, INDIVIDUAL allocation) depends on whether Free is monthly or lifetime, and whether Viewer/Admin are Metronome seats.
> 
...
| `workspace_gauge` | Daily Temporal cron | Once/day/workspace — carries all gauge properties (member_count, mau_1/5/10, data sources, storage, feature flags) in a single event |

## Seats and auto-assigned credits

When a workspace subscribes:

1. Contract created from package → seat subscriptions (Free, Pro, Max) provisioned automatically
2. All existing members bulk-assigned as **Free seats** ($0, 300 AWU/month)
3. Each seat gets its monthly credit allocation via `INDIVIDUAL` recurring credits in the package

On membership changes:

- **Member joins** → `addMetronomeFreeSeat` (fire-and-forget from `createMembership`)
...
Free seats are a **$0 Metronome seat product** with 300 AWU/month INDIVIDUAL recurring credits — same mechanism as Pro/Max, just at $0. This keeps all credit allocations in Metronome (single source of truth), makes the 300 credit limit configurable per contract (e.g., enterprise deal with 500 free credits), and avoids hardcoding limits in code.

Seat product IDs are resolved dynamically from Metronome's products API by name — no hardcoded IDs.

> **OPEN — seat upgrade policy.** Since Free seats cost $0 and include 300 AWU, it makes sense to default all new members to Free rather than Pro. This means the workspace only pays for users who actually need more credits. Two questions:
> 
> 1. **Free → Pro upgrade:** Should this be automatic (e.g., when a user exhausts their 300 free credits, auto-upgrade to Pro and start billing $30/mo) or manual (admin must explicitly upgrade the user, and the user is blocked until then)?
> 2. **Pro → Max upgrade:** Should this be automatic (when a Pro user exhausts 5,000 credits) or always manual (admin decision, since Max is 4× the price)?
> 
> Auto-upgrade is smoother UX but creates surprise charges for admins. Manual upgrade gives admins cost control but blocks users mid-workflow. A hybrid (auto Free→Pro, manual Pro→Max) may be the right balance. Needs product decision.
> 
...
Cons: rounding is per-period not per-message (10 × 0.3 AWU = 3 AWU, not 10); requires maintaining ~60 models × 4 token types = ~240 rate card entries in Metronome; no bulk import (no CSV/table upload — each rate entry must be added via API call or UI one by one); new models need rate card entries or usage won't be billed; high operational burden on GTM/Ops for model additions.

> **OPEN — needs decision.** These two options are **not equivalent** and cannot be switched after launch without changing billing behavior. Per-message rounding (A) systematically charges more than per-period rounding (B) for users with many small messages. The choice also determines where per-model pricing lives (code vs Metronome) and who can change it (engineers vs GTM/Ops).
> 
...
- **User usage** (`is_programmatic_usage=false`, has `user_id`): Web/Slack/extension. Consumed from the user's **individual seat allocation** first (Free/Pro/Max), then falls back to workspace pool
...
| `parent_agent_message_id` | string (optional) | Parent message sId when `is_sub_agent_message=true`. Allows grouping all costs from an agent chain under the original user action |
...
| `cost_micro_usd` | number | Provider cost (no markup), from `token_pricing.ts`. Used by legacy rate cards (30% markup in USD). Not used for AWU billing on new plans — kept for analytics, margin tracking, and shadow mode validation |
| `awu_credits` | number | Intelligence AWU for this LLM call, `ceil(cost_usd / 0.01)`. 0 for basic messages. Used by new pricing Option A only |
...
| `workspace_gauge` | `workspace-gauge-{workspaceSId}-{YYYY-MM-DD}` | One per workspace per day — all gauges in a single event |
...
All metrics feed from the same events. Legacy and new pricing use **different metrics** based on different aggregation keys.

### Legacy metrics (based on `cost_micro_usd`, programmatic only)

| Billable Metric | Aggregation | Filter | Used by |
| --- | --- | --- | --- |
| LLM Provider Cost (Programmatic) | SUM on `cost_micro_usd` | `is_programmatic_usage=true` | Legacy rate cards (30% markup in USD) |
| Registered Users | MAX on `member_count` | event_type=`workspace_gauge` | Legacy plans (bracket-based free credit commit) |
...
Depends on chosen pricing option (see Pricing logic section):

**Option A — AWU computed in code:**

| Billable Metric | Aggregation | Filter | Notes |
| --- | --- | --- | --- |
| AI Usage AWU (Programmatic) | SUM on `awu_credits` | `is_programmatic_usage=true` | Pre-rounded integer AWU per LLM call |
| AI Usage AWU (User) | SUM on `awu_credits` | `is_programmatic_usage=false` | Pre-rounded integer AWU per LLM call |
| Tool Invocations (Programmatic) | COUNT (succeeded only) | grouped by `tool_category` | Shared with legacy (same metric, different rate card prices) |
| Tool Invocations (User) | COUNT (succeeded only) | grouped by `tool_category` | New pricing only |

**Option B — dimensional pricing in Metronome:**

| Billable Metric | Aggregation | Filter | Notes |
| --- | --- | --- | --- |
| Prompt Tokens | SUM on `prompt_tokens` | grouped by `model_id` | Per-model per-token pricing in rate card |
| Completion Tokens | SUM on `completion_tokens` | grouped by `model_id` | Per-model per-token pricing in rate card |
| Cached Tokens | SUM on `cached_tokens` | grouped by `model_id` | Typically discounted or free |
| Cache Creation Tokens | SUM on `cache_creation_tokens` | grouped by `model_id` | Typically same price as prompt tokens |
| Tool Invocations (Programmatic) | COUNT (succeeded only) | grouped by `tool_category` | Shared with legacy |
| Tool Invocations (User) | COUNT (succeeded only) | grouped by `tool_category` | New pricing only |

> **Note:** Option B requires ~4 new metrics (one per token type) with `model_id` group key, plus ~240 rate card entries. Option A reuses simpler metrics but requires code-side AWU computation. Tool metrics are the same for both options — only intelligence metrics differ.
> 

### Metronome products

### Legacy products (USD-priced, programmatic only)

| Product | Type | Metric | Pricing Key | Notes |
| --- | --- | --- | --- | --- |
| AI Usage (Programmatic) | USAGE | LLM Provider Cost (Programmatic) | — | quantity_conversion ÷1M (micro-USD → dollars). Priced in USD with 30% markup |
...
| MAU Reporting | USAGE | MAU_1/5/10 | — | For MAU-based enterprise contracts only |

### New pricing products (AWU-priced, programmatic + user)
...
| Product | Type | Metric | Pricing Key | Notes |
| --- | --- | --- | --- | --- |
| AI Usage (Programmatic) | USAGE | AI Usage AWU (Programmatic) | — | SUM of pre-rounded `awu_credits`. Priced in AWU |
| AI Usage (User) | USAGE | AI Usage AWU (User) | — | SUM of pre-rounded `awu_credits`. Priced in AWU |
| Tool Usage (Programmatic) | USAGE | Tool Invocations (Programmatic) | `tool_category` | Dimensional pricing in AWU by tool weights |
| Tool Usage (User) | USAGE | Tool Invocations (User) | `tool_category` | Dimensional pricing in AWU by tool weights |

**Option B — dimensional pricing in Metronome:**

| Product | Type | Metric | Pricing Key | Notes |
| --- | --- | --- | --- | --- |
| Prompt Token Usage | USAGE | Prompt Tokens | `model_id` | Per-model per-token pricing in AWU |
| Completion Token Usage | USAGE | Completion Tokens | `model_id` | Per-model per-token pricing in AWU |
| Cached Token Usage | USAGE | Cached Tokens | `model_id` | Typically discounted or free |
| Cache Creation Token Usage | USAGE | Cache Creation Tokens | `model_id` | Typically same price as prompt tokens |
| Tool Usage (Programmatic) | USAGE | Tool Invocations (Programmatic) | `tool_category` | Dimensional pricing in AWU by tool weights |
| Tool Usage (User) | USAGE | Tool Invocations (User) | `tool_category` | Dimensional pricing in AWU by tool weights |

**Seat and credit products (shared across Option A and B):**

| Product | Type | Notes |
| --- | --- | --- |
| Free Seat | SUBSCRIPTION | $0/mo + 300 AWU/month (INDIVIDUAL) |
...
> **Note:** Tool Usage products are the same for both options — tool invocations are integer counts with AWU weights, no rounding issue. Only intelligence (LLM) products differ between options.
> 
...
| Product | Price | Notes |
| --- | --- | --- |
| Free Seat | $0/mo | FLAT + MONTHLY, includes 300 AWU/month (INDIVIDUAL) |
...
| AI Usage (Programmatic) | 1 AWU per unit | Metric already produces AWU (via `awu_credits` or dimensional pricing). Rate card is pass-through |
| AI Usage (User) | 1 AWU per unit | Same — metric already in AWU |
...
> **Note:** Intelligence markup can be applied at multiple levels: in code (Option A: baked into `awu_credits` computation), in the rate card (Option B: per-model token prices include markup), or via the AWU-to-USD conversion rate (`fiat_per_custom_credit`) which can be tuned per contract — e.g., $0.01/AWU at list price but $0.008/AWU for a high-volume enterprise deal. The rate card AI Usage price is a simple 1:1 pass-through since the metric already outputs AWU. Tool credit weights still being calibrated.
> 
> 
...
**Pending changes depending on pricing option choice:**

- **Option A**: `events.ts` must compute `awu_credits` per LLM call (`ceil(cost_usd / 0.01)`, 0 for basic messages) and include it in `llm_usage` events
- **Option B**: no change to events (raw token counts already sent)
...
| **Seat/access model** (Free monthly vs lifetime, Viewer/Admin/Light seat types) | Metronome package config, seat products, subscription flow | Product |
...
| Task | Description | Dependencies |
| --- | --- | --- |
| **E1. Add `awu_credits` to events** | Compute `ceil(cost_usd / 0.01)` per LLM call, add to `llm_usage` events. 0 for basic messages. Only needed for Option A | F4, pricing option decision |
...
| Task | Description | Dependencies |
| --- | --- | --- |
| **M1. Create new pricing metrics in production** | AWU metrics (Option A: `awu_credits` SUM; Option B: per-token-type with `model_id` group key) | Pricing option decision |
| **M2. Create legacy metrics in production** | LLM Provider Cost (Programmatic), Registered Users, MAU_1/5/10 | None |
| **M3. Create all products in production** | Legacy seats, new seats (Free/Pro/Max), AI Usage, Tool Usage, AWU Credits | M1, M2 |
