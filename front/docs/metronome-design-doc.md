...
| --- | --- | --- |
| Plan (e.g., Business Plan) | - | Plan defines features/limits only. Billing backend (Stripe vs Metronome) is determined by the Subscription, not the Plan |
| Workspace | Customer | 1:1 — `metronomeCustomerId` on `WorkspaceModel`, workspace `sId` as ingest alias |
| Subscription | Contract | 1:1 — `metronomeContractId` on `SubscriptionModel`. Same pattern as `stripeSubscriptionId` |
...
A user with exhausted seat credits can still send messages if the workspace pool has balance.
    ...
    ```
    if isMetronomeBilled(subscription):
...
`SubscriptionPage` detects `metronomeContractId` on the active subscription and conditionally renders:
...
**Usage events** (per-message, via `emitMetronomeUsageEventsWorkflow`):

- `front/temporal/usage_queue/activities.ts` — `emitMetronomeUsageEventsActivity` queries RunUsages and MCP actions, classifies message tier, builds and ingests events
- **One `llm_usage` event per (provider_id, model_id) group** — aggregates token counts and cost per model for dimensional pricing
- **One `tool_use` event per (tool_name, mcp_server_id, status) group** — identical invocations aggregated with `count` and `total_execution_duration_ms`
- **All messages** (parent and sub-agent) emit events. Sub-agent detection via `agenticMessageType` / `agenticOriginMessageId` on the user message
- **Gated by `metronome_billing` feature flag** — no workflow started if the flag is not set on the workspace
- Errors propagate to Temporal for retry (default exponential backoff). Deterministic `transaction_id`s ensure idempotent retries. Events batched into chunks of 100 (Metronome's max per request)

**Gauge events** (daily, via `emitMetronomeGaugeEventsWorkflow`):

- `front/temporal/usage_queue/activities.ts` — `emitMetronomeGaugeEventsForAllWorkspacesActivity` iterates all workspaces (concurrency 10), emits one `workspace_gauge` event per workspace
- Properties: `member_count`, `mau_1_count`, `mau_5_count`, `mau_10_count`
- Schedule started on worker boot (`launchMetronomeGaugeEventsSchedule`): daily in prod, hourly in dev, SKIP overlap policy
- Idempotent: same `dateKey` → same `transaction_id` → Metronome deduplicates

Events are built by `front/lib/metronome/events.ts` and ingested via `front/lib/metronome/client.ts` → `ingestMetronomeEvents` (using `@metronome/sdk`).
...
- `metronome-finalize` provisions Metronome customer + contract, stores `metronomeContractId` on the new subscription, bulk-assigns Guest seats

**Production (legacy, shadow mode):**

- Standard `SubscribePage` with legacy plans (Pro $29, Business $39) → Stripe Checkout with recurring subscription (`stripeSubscriptionId` on subscription)
- All usage events double-written to Metronome (shadow mode)
- Poke upgrade path: `front/pages/api/poke/workspaces/[wId]/upgrade.ts`

**Full rollout (future):**

- `SubscribePage` shows Metronome-billed plans → Stripe setup checkout → `metronome-finalize` → subscription created with `metronomeContractId` (no `stripeSubscriptionId`)
- Same plan (e.g., Pro $29) can be billed via Stripe (legacy subscriptions) or Metronome (new subscriptions)
...
```tsx
function isMetronomeBilled(subscription: SubscriptionType): boolean {
  return subscription.metronomeContractId != null;
}
```

Billing backend is determined by the **Subscription**, not the Plan:

- `stripeSubscriptionId` set → Stripe billing
- `metronomeContractId` set → Metronome billing
- Neither → free/no billing

The Plan defines features and limits only. The same plan (e.g., Pro $29) can be billed through either Stripe or Metronome depending on when/how the subscription was created.

**Shadow mode:** All workspaces keep `stripeSubscriptionId` (billing on Stripe). Events flow to Metronome for comparison. When migrating a workspace to Metronome, a new subscription is created with `metronomeContractId` (and no `stripeSubscriptionId`).
...
- **Phase 5 (new pricing):** New signups get subscriptions with `metronomeContractId` pointing to a `business-plan` contract. Existing workspaces can be migrated by creating a new subscription with a new Metronome contract
...
| --- | --- | --- |
| **F1. Data model** | Add `metronomeContractId` to `SubscriptionModel` + `metronomeCustomerId` to `WorkspaceModel`. DB migrations. `isMetronomeBilled(subscription)` helper | None |
| **F2. Plan codes** | ~~No longer needed — plans are billing-agnostic. Package alias is passed at checkout time, not stored on the plan~~ | Removed |
...
| **F8. Subscription resource changes** | Handle `metronomeContractId` on subscription creation. Skip Stripe cancellation for Metronome subscriptions. `ended_backend_only` handling | F9 |
| **F9. Fix data model**  | Move billing backend from Plan to Subscription: remove `metronomePackageAlias` from PlanModel, add `metronomeContractId` to SubscriptionModel. Update `isMetronomeBilled(subscription)`  | F1 |
...
| **E4. Skip Stripe usage reporting for Metronome workspaces** | `reportUsageForSubscriptionItems` + `trackProgrammaticCost`: skip when `isMetronomeBilled(subscription)` | F5 |
...
| **U2. Subscription page conditional rendering** | Detect `metronomeContractId` on active subscription: show Metronome section vs Stripe portal | F1 |
