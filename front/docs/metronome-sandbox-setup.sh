...
# pricing_group_key: different AWU price per tool_category
# presentation_group_key: all categories grouped into single "Tool Usage" invoice line
# tool_group is a constant "tools" property on every tool_use event
...
# =============================================================================
# PRODUCTS — New pricing: Seat types
# =============================================================================

# --- Business plan seats ---

curl -s -X POST "$API/contract-pricing/products/create" -H "$AUTH" -H "$CT" -d '{
  "name": "Guest Seat", "type": "SUBSCRIPTION"
}'
# -> id: f57641dd-d28f-4d32-9849-c789fcb0502d
# $0/mo, 0 AWU. Default when user joins Business workspace. Appears on invoice as $0 line item.

curl -s -X POST "$API/contract-pricing/products/create" -H "$AUTH" -H "$CT" -d '{
  "name": "Pro Seat", "type": "SUBSCRIPTION"
}'
# -> id: b852890e-bf8e-44ea-add7-1910af5c9151
# $24/yr ($30/mo) + 5,000 AWU/month (INDIVIDUAL)

curl -s -X POST "$API/contract-pricing/products/create" -H "$AUTH" -H "$CT" -d '{
  "name": "Max Seat", "type": "SUBSCRIPTION"
}'
# -> id: 68ae2029-c592-4a21-bd20-4d6bab7be9b8
# $100/yr ($125/mo) + 20,000 AWU/month (INDIVIDUAL)

# --- Enterprise Platform plan seats ---

curl -s -X POST "$API/contract-pricing/products/create" -H "$AUTH" -H "$CT" -d '{
  "name": "Light Seat", "type": "SUBSCRIPTION"
}'
# -> id: 13999c46-5f3e-40ed-9426-bb6aa88082d7
# Enterprise Platform only. Limited features, credits from workspace pool.

curl -s -X POST "$API/contract-pricing/products/create" -H "$AUTH" -H "$CT" -d '{
  "name": "Standard Seat", "type": "SUBSCRIPTION"
}'
# -> id: 2ef07112-5e01-4df8-8474-bcbdecfbb2e3
# Enterprise Platform only. Full Builder access, credits from workspace pool.
...
#
# Products — New pricing (Business plan seats):
#   Guest Seat:                          f57641dd-d28f-4d32-9849-c789fcb0502d
#   Free Seat:                           ARCHIVED (de5685d1-6acb-4042-8091-412e7c0c2edb)
#   Pro Seat:                            b852890e-bf8e-44ea-add7-1910af5c9151
#   Max Seat:                            68ae2029-c592-4a21-bd20-4d6bab7be9b8
#
# Products — New pricing (Enterprise Platform seats):
#   Light Seat:                          13999c46-5f3e-40ed-9426-bb6aa88082d7
#   Standard Seat:                       2ef07112-5e01-4df8-8474-bcbdecfbb2e3
...
#   business-plan:                       7ee772e0-21b5-4870-b6f1-5574149f52d9
#
...
#   Guest Seat:               0 USD cents ($0/mo, 0 AWU)
...
#   Guest Seat:     0 AWU/month (no recurring credits)
#   Pro Seat:   5,000 AWU/month (INDIVIDUAL allocation)
#   Max Seat:  20,000 AWU/month (INDIVIDUAL allocation)
#
# Enterprise Platform plan seats (Light, Standard) are not in the Business Plan package.
# They would be in a separate enterprise-plan package with pooled workspace credits.
#
# =============================================================================
# PHASE 2: Dimensional Pricing (Metronome Pricing Index) — EXPERIMENTAL
# =============================================================================
#
# Per-model per-token metrics and products for future migration to Metronome
# Pricing Index. Not used for billing yet — created for testing/validation.
#
# Billable Metrics (Phase 2):
#   Prompt Tokens (by model_id):         82c85670-e4ae-4c7c-8c99-03c777d3eb58
#   Completion Tokens (by model_id):     fae0a934-f2d5-45a0-82e5-7f79f9d01cd4
#   Cached Tokens (by model_id):         f0e40bc0-0793-471e-b696-1f58c2966695
#   Cache Creation Tokens (by model_id): ba912c99-aca3-4629-9c09-012fbddac56a
#
# Products (Phase 2):
#   Prompt Token Usage:                  00cce4f3-507c-4024-acaf-0ac020fad56d
#   Completion Token Usage:              a009284e-eaea-4c2a-8182-3f0cec9ffa15
#   Cached Token Usage:                  c9e1269c-0d61-4d96-87de-2c9dde84c278
#   Cache Creation Token Usage:          fec1a119-b8e3-48fb-9dd3-ee1f463ae9fe
#
# Rate Card (Phase 2):
#   Phase 2 - Pricing Index:             f455240c-7f98-4c90-8d4f-42df8aa535f6
#
# Sample per-model rates (AWU per 1M tokens, 1 AWU = $0.01):
#
#   Model                  | Prompt | Completion | Cached | Cache Write
#   -----------------------|--------|------------|--------|------------
#   claude-sonnet-4-6      |    300 |      1,500 |     30 |         375
#   claude-opus-4-6        |  1,500 |      7,500 |    150 |       1,875
#   claude-haiku-4-5       |     80 |        400 |      8 |         100
#   gpt-4o                 |    250 |      1,000 |    125 |           —
#   gpt-4o-mini            |     15 |         60 |      8 |           —
#   o3                     |    200 |        800 |      — |           —
#
# To use: create a contract with this rate card instead of "Business Plan".
# Events already send model_id + token counts — no code changes needed.
