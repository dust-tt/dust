...
## Cross-mount implementation — 2026-08-11

The adapter now implements that unified topology. The two CAB-scoped gcsfuse mounts remain hidden under `/run/dust-fs/data`, but one `dust-fs-overlay` instance mounts at `/files` and routes the canonical conversation and pod directory names to their respective backing. Legacy `/files/conversation` and `/files/pod` entries are synthetic symlinks inside the same FUSE filesystem. As a result, Linux can submit conversation-to-pod and pod-to-conversation moves as one rename callback instead of falling back to copy plus unlink.

The rename mutation gained an optional `destinationMount`. Keeping it optional preserves compatibility with already-running single-mount helpers, where omission continues to mean the source mount. The sandbox endpoint authorizes both mount identities against the short-lived token. Front then uses the conversation-scoped `DustFileSystem` containing its associated pod and reuses `moveCanonicalFile`, so the existing frame sId, mount path, use case metadata, bundle paths, and publication are reconciled together.

Local verification covers the routed overlay self-test, one-overlay/two-backing startup order, legacy aliases, same-mount request compatibility, destination token scope, and a functional conversation-to-pod frame move that retains the `FileResource` identity. The new helper is assigned `dust-base:0.8.77`. The remaining operational gate is building that E2B image and rerunning `scripts/test_sandbox_cross_mount_frame.ts`; the current development shell lacks the configured artifact registry and service-account settings, so no remote build was started from this worktree yet.
