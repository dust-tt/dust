...
import {
  isSandboxToolExecutionInClaimsSession,
...
import type { SandboxToolExecutionType } from "@app/types/actions";
...
async function executeAndRespond(
  ...
  action: SandboxToolExecutionType,
  ...
  try {
    ...
    await AgentMCPActionResource.updateSandboxToolExecutionStatus(
      auth,
      action.sId,
      result.isError ? "errored" : "succeeded"
    );
    res.status(200).json({
      success: true,
      result: {
...
async function handlePollingPost(
  ...
  const action = await AgentMCPActionResource.fetchSandboxToolExecutionById(
    auth,
    actionId
  );
  if (
    !action ||
    !(await isSandboxToolExecutionInClaimsSession(auth, sandboxClaims, action))
  ) {
    return apiError(req, res, {
      status_code: 404,
      api_error: {
...
async function handleInitialToolCall(
  ...
  const { status: stakeStatus } = await getExecutionStatusFromConfig(auth, {
    actionConfiguration: runContext.toolConfiguration,
  ...
  // The bash tool action must exist — `sandbox_tool_executions` carry a
  // NOT NULL FK to the parent. The sandbox client only ever calls us from
  // inside an active bash tool, so missing it is a programming error.
  const { parentAction } = sandboxResult;
  if (!parentAction) {
    return apiError(req, res, {
      status_code: 500,
      api_error: {
        type: "internal_server_error",
        message: "Sandbox tool call has no parent agent action.",
      },
    });
  }
  ...
  const action = await parentAction.spawnChildSandboxToolExecution({
    status: stakeStatus,
