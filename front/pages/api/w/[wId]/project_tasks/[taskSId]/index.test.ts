...
describe("GET /api/w/[wId]/project_tasks/[taskSId]", () => {
  ...
  it("returns 404 for an unknown todo sId", async () => {
    await setup();
    req.query.taskSId = "nonexistent_todo_sid";

    await handler(req, res);

    expect(res._getStatusCode()).toBe(404);
    expect(res._getJSONData().error.type).toBe("project_task_not_found");
  });

  it("returns 400 when taskSId query param is missing", async () => {
    await setup();
