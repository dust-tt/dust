...
describe("GET /api/w/[wId]/assistant/conversations/search", () => {
  ...
  describe("Success Cases", () => {
    ...
    it("handles chunks without scores (defaults to 0)", async () => {
      const { req, res, workspace, user, authenticator, globalGroup } =
        await createPrivateApiMockRequest({
          method: "GET",
          role: "admin",
      ...
      const mockDocuments: CoreAPIDocument[] = [
        {
          ...
          chunk_count: 1,
          chunks: [
            { text: "test", hash: "chunk-hash", offset: 0, score: null },
          ],
          title: "Doc 1",
          mime_type: null,
        },
    ...
    it("respects limit parameter", async () => {
      ...
      vi.spyOn(CoreAPI.prototype, "bulkSearchDataSources").mockResolvedValue(
        new Ok({
          documents: [
            createMockDocument(mockDataSourceId, conv1.sId, 0.9),
            createMockDocument(mockDataSourceId, conv2.sId, 0.8),
            createMockDocument(mockDataSourceId, conv3.sId, 0.7),
          ],
        })
      );
      ...
      expect(data.conversations).toHaveLength(2);
      expect(data.conversations[0].sId).toBe(conv1.sId);
      expect(data.conversations[1].sId).toBe(conv2.sId);
    });
    ...
    it("includes spaceName in returned conversations", async () => {
      const { req, res, workspace, user, authenticator, globalGroup } =
        await createPrivateApiMockRequest({
          method: "GET",
          role: "admin",
      ...
      vi.spyOn(CoreAPI.prototype, "bulkSearchDataSources").mockResolvedValue(
        new Ok({
          documents: [createMockDocument(mockDataSourceId, conv1.sId, 0.9)],
        })
      );
      ...
      const data = res._getJSONData();
      expect(data.conversations).toHaveLength(1);
      expect(data.conversations[0].spaceName).toBe(projectSpace.name);
    });
