...
import { _searchServerForAttachments } from "@app/lib/actions/mcp_attachments";
  ...
  try {
    ...
    const results = await Promise.all(
      attachmentServerViews.map(async (serverView) => {
        ...
        const serverNodes = await _searchServerForAttachments({
          auth,
          serverId,
          query,
          pageSize,
        ...
        return serverNodes.map((node) => ({
          ...
          serverName: serverView.name ?? "Unknown",
        }));
      })
    );
