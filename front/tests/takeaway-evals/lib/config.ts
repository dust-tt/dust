...
export const RUN_TAKEAWAY_EVAL = process.env.RUN_TAKEAWAY_EVAL === "true";
...
/** Model used for the takeaway extraction LLM calls. */
function resolveModelId(): ModelIdType {
  const id =
    process.env.TAKEAWAY_MODEL_ID ||
    CLAUDE_4_5_HAIKU_DEFAULT_MODEL_CONFIG.modelId;
  if (!isModelId(id)) {
    throw new Error(
      `Invalid TAKEAWAY_MODEL_ID: "${id}". Must be a known model ID.`
    );
  }
  return id;
}
...
export async function getTakeawayEvalAuth(): Promise<Authenticator> {
