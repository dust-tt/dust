...
Score 2-3 if directives are present and the response clearly explains what was suggested (instructions for bug collection/formatting and GitHub tool).`,
    },
    {
      ...
      judgeCriteria: `Intent is vague (do NOT suggest until intent is clear). Should clarify via ask_user_question — one pinpointed question (purpose, audience, or capabilities) with relevant options is sufficient and correct. Should NOT suggest instructions yet. Score 0-1 if sidekick suggests instructions without clarifying first. Accept a response with no text and only the question.`,
    },
    {
      ...
      isFirstMessage: true,
      expectedToolCalls: [],
      judgeCriteria: `Intent is vague. Should clarify via ask_user_question — one pinpointed question (what team, what kind of help, or current problems) with relevant options is sufficient and correct. Should NOT suggest instructions until clarified. Score 0-1 if sidekick suggests instructions without understanding use case.`,
    },
    {
      ...
      isFirstMessage: true,
      expectedToolCalls: [],
      judgeCriteria: `Intent is partially clear (sales team). Should narrow down via ask_user_question — one pinpointed question (struggles, tools, or process) with relevant options is sufficient and correct. May suggest patterns (lead research, email drafter). Should NOT suggest full instructions without discovery. Score 0-1 if generic sales instructions without clarification.`,
    },
    {
      ...
      isFirstMessage: true,
      expectedToolCalls: [],
      judgeCriteria: `Intent is vague (marketing help). Should run discovery via ask_user_question — one pinpointed question (pain point, content type, workflow, or tools) with relevant options is sufficient and correct. May suggest agent ideas. Score 0-1 if sidekick suggests generic marketing instructions without discovery, or if the question is generic enough to be useless.`,
    },
  ],
};
