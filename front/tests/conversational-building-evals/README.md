...
query. `agents` are created with hand-written block HTML (`instructions-root` div holding
`instruction-block` sections) whose `data-block-id`s the scenario picks, so a
