...
export function ScrollAreaDemo() {
  return (
    <ScrollArea className="s-border-border s-h-72 s-w-48 s-border-b s-border-t">
      <div className="s-p-4">
        <h4 className="s-mb-4 s-text-sm s-font-medium s-leading-none">Tags</h4>
        {tags.map((tag) => (
      ...
      <div className="s-flex s-w-max s-space-x-4 s-p-4">
        {works.map((artwork) => (
          <figure key={artwork.artist} className="s-shrink-0">
            ...
            <figcaption className="s-text-muted-foreground s-pt-2 s-text-xs">
              Photo by{" "}
              <span className="s-text-foreground s-font-semibold">
                {artwork.artist}
              </span>
            </figcaption>
          </figure>
