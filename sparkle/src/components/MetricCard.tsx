...
interface MetricCardRootProps extends BaseProps, VariantProps<typeof metricCardVariants> {}
...
const Title: React.FC<MetricCardTitleProps> = ({ className, children }) => {
  return (
    <div className={cn(STYLES.text.title, className)}>
      {children}
    </div>
  );
};

const Subtitle: React.FC<MetricCardSubtitleProps> = ({ className, children }) => {
  return (
    <div className={cn(STYLES.text.subtitle, className)}>
      {children}
    </div>
  );
};
...
const Content: React.FC<MetricCardContentProps> = ({ 
  ...
  return (
    <div className={cn(STYLES.layout.content, className)}>
      {children}
    </div>
  );
};
...
const Footer: React.FC<MetricCardFooterProps> = ({ className, children }) => {
  return (
    <div className={cn(STYLES.layout.footer, className)}>
      {children}
    </div>
  );
};
