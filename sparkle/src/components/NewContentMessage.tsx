...
const NEW_CONTENT_MESSAGE_VARIANTS = [
  "error",
  "success",
  "info",
  "warning",
...
type NewContentMessageVariantType =
  (typeof NEW_CONTENT_MESSAGE_VARIANTS)[number];

const NEW_CONTENT_MESSAGE_SIZES = ["sm", "xs"] as const;

type NewContentMessageSizeType = (typeof NEW_CONTENT_MESSAGE_SIZES)[number];
...
export interface NewContentMessageProps {
  title?: string;
  children?: React.ReactNode;
  className?: string;
  size?: NewContentMessageSizeType;
  variant?: NewContentMessageVariantType;
  icon?: ComponentType;
  action?: React.ReactNode;
}

function NewContentMessage({
  title,
  variant = "info",
  children,
  size = "sm",
...
function NewContentMessageAction(props: ButtonProps) {
  return (
    <Button size="xs" className={cn("shrink-0", props.className)} {...props} />
  );
}

export { NewContentMessage, NewContentMessageAction };
