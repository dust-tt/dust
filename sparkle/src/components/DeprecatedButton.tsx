...
export type DeprecatedButtonVariantType =
  (typeof DEPRECATED_BUTTON_VARIANTS)[number];

export const DEPRECATED_REGULAR_BUTTON_SIZES = [
  "xmini",
  "mini",
  "xs",
  "sm",
...
export const DEPRECATED_ICON_ONLY_SIZES = [
  "icon-xs",
  "icon",
  "icon-sm",
] as const;
export const DEPRECATED_SMALL_BUTTON_SIZES = [
  "icon-xs",
  "icon",
  "xmini",
  "mini",
...
export type DeprecatedRegularButtonSize =
  (typeof DEPRECATED_REGULAR_BUTTON_SIZES)[number];
export type DeprecatedIconOnlySize =
  (typeof DEPRECATED_ICON_ONLY_SIZES)[number];
export type DeprecatedButtonSize =
  | DeprecatedRegularButtonSize
  | DeprecatedIconOnlySize;

function isSmallButtonSize(
  size: DeprecatedButtonSize | undefined
): size is (typeof DEPRECATED_SMALL_BUTTON_SIZES)[number] {
  return (
    size !== undefined &&
    DEPRECATED_SMALL_BUTTON_SIZES.includes(
      size as (typeof DEPRECATED_SMALL_BUTTON_SIZES)[number]
    )
  );
}

// Define button styling with cva
const deprecatedButtonVariants = cva(
  cn(
...
const spinnerVariantsMap: Record<DeprecatedButtonVariantType, SpinnerVariant> =
  {
    primary: "revert",
    highlight: "light",
...
export interface DeprecatedMetaButtonProps
  extends React.ButtonHTMLAttributes<HTMLButtonElement>,
    VariantProps<typeof deprecatedButtonVariants> {
  asChild?: boolean;
  isRounded?: boolean;
}

const DeprecatedMetaButton = React.forwardRef<
  HTMLButtonElement,
  DeprecatedMetaButtonProps
>(
  (
    {
      className,
    ...
    return (
      <Comp
        className={cn(
          deprecatedButtonVariants({ variant, size, rounded, className })
        )}
        ref={ref}
        {...props}
      >
...
DeprecatedMetaButton.displayName = "DeprecatedMetaButton";

type IconSizeType = "xs" | "sm" | "md";
type CounterSizeType = "xs" | "sm" | "md";

export const DEPRECATED_ICON_SIZE_MAP: Record<
...
const COUNTER_SIZE_MAP: Record<DeprecatedButtonSize, CounterSizeType> = {
  "icon-xs": "xs",
  icon: "xs",
  "icon-sm": "sm",
  xmini: "xs",
...
type CommonButtonProps = Omit<DeprecatedMetaButtonProps, "children"> &
...
export type DeprecatedButtonIconType = React.ComponentType | React.ReactElement;

function isReactElement(
  visual: DeprecatedButtonIconType
): visual is React.ReactElement {
  return React.isValidElement(visual);
}

export type DeprecatedIconOnlyButtonProps = CommonButtonProps & {
  size: DeprecatedIconOnlySize;
  icon: DeprecatedButtonIconType;
  label?: never;
};

export type DeprecatedRegularButtonProps = CommonButtonProps & {
...
export type DeprecatedButtonProps =
  | DeprecatedIconOnlyButtonProps
  | DeprecatedRegularButtonProps;

const DeprecatedButton = React.forwardRef<
  HTMLButtonElement,
...
export { DeprecatedButton, DeprecatedMetaButton, deprecatedButtonVariants };
