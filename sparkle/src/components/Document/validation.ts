...
import { z } from "zod";
import { DOCUMENT_MAX_FRAMES, DocumentFrame } from "./DocumentFrame";
...
export const validateDocumentJSON = (
  ...
  while (pending.length > 0) {
    ...
    if (node.type === DocumentFrame.name) {
      frames += 1;
      if (frames > DOCUMENT_MAX_FRAMES) {
        return {
          ok: false,
          error: `A document can contain at most ${DOCUMENT_MAX_FRAMES} Frames.`,
        };
      }
    }
