...
function Projects_Panel() {
  ...
  const selectedSpaceId = p2View.kind === "space" ? p2View.spaceId : null;
  ...
  const spaceConversations = useMemo(
    () => (selectedSpaceId ? getConversationsBySpaceId(selectedSpaceId) : []),
    [selectedSpaceId]
  );
  ...
  const p2Content = (() => {
    ...
    if (p2View.kind === "space" && selectedSpace && selectedSpaceId)
      return (
        <GroupConversationView
          ...
          editorUserIds={spaceEditors.get(selectedSpaceId) ?? []}
  ...
  const spaceTopBarRight = (() => {
    if (p2View.kind !== "space" || !selectedSpaceId) return null;
    const memberIds =
      spaceMembers.get(selectedSpaceId) ?? getMembersBySpaceId(selectedSpaceId);
  ...
  const p2TopBarLeft = (() => {
    ...
    if (p2View.kind === "space") return spaceTopBarLeft;
  ...
  const p2TopBarRight = (() => {
    ...
    if (p2View.kind === "space") return spaceTopBarRight;
    return null;
  })();
