...
// ---------------------------------------------------------------------------
// Story component — AgentMessage_UserValidation playground (duplicate of Conversation)
// ---------------------------------------------------------------------------

export default function AgentMessage_UserValidationStory() {
  const baseConversation = useMemo(() => buildConversation(), []);

  const [conversation, setConversation] = useState<Conversation>(() => {
    const now = new Date();
    const pendingBlock = createPendingValidationBlock(now);
    const messages: ConversationItem[] = [
  ...
  const handleAcceptPendingValidation = useCallback((blockId: string) => {
    setConversation((prev) => {
      const messages = prev.messages ?? [];
      const blockIndex = messages.findIndex(
        (item) => item.kind === "pendingValidation" && item.id === blockId
      );
      if (blockIndex < 0) return prev;

      const block = messages[blockIndex] as ConversationPendingValidation;
      const newMessages: ConversationItem[] = [
        ...messages.slice(0, blockIndex),
        block.userMessage,
        block.agentMessage,
        ...messages.slice(blockIndex + 1),
      ];
