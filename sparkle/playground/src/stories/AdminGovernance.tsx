...
interface MemberRow {
  ...
  name: string;
  email: string;
  plan: "owner" | "super_admin" | "admin" | "member";
...
const PLAN_LABELS: Record<MemberRow["plan"], string> = {
  owner: "Owner",
  super_admin: "Super Admin",
  admin: "Admin",
  member: "Member",
...
function MemberPickerSheet({
  ...
  const filtered = SHEET_MEMBERS.filter(
    (m) =>
      !search ||
                ...
                {filtered.map((m) => (
                  ...
                  >
                    ...
                    <td className="s-py-3 s-text-right">
                      <Chip
                        color={m.plan === "owner" ? "green" : "blue"}
                        label={m.plan === "owner" ? "Owner" : "Member"}
                        size="sm"
                      />
                    </td>
                  </tr>
  ...
  const memberColumns = useMemo<ColumnDef<MemberRow>[]>(
    () => [
      ...
      },
      {
        ...
        cell: (info) => {
          const plan = info.getValue() as MemberRow["plan"];
          return (
            <DataTable.CellContent>
              <Chip
