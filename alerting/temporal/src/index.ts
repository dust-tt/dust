...
const { METRICS_CLIENT_CERT, METRICS_CLIENT_KEY, TEMPORAL_CLOUD_BASE_URL } = process.env;
...
export const labelsResponseDataSchema = z.object({
...
const getMetricNames = async () => {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            const countMetricNames = metricNames.filter((metricName) => TARGET_METRIC_NAMES.includes(metricName) &&
                metricName.endsWith("_count"));
            const histogramMetricNames = metricNames.filter((metricName) => TARGET_METRIC_NAMES.includes(metricName) &&
...
const alignQueryWindowOnPrometheusStep = (queryWindow: QueryWindow): QueryWindow => {
    const startSecondsSinceEpoch = Math.floor(queryWindow.startSecondsSinceEpoch / PROMETHEUS_STEP_SECONDS - 1) * PROMETHEUS_STEP_SECONDS;
    const endSecondsSinceEpoch = Math.floor(queryWindow.endSecondsSinceEpoch / PROMETHEUS_STEP_SECONDS + 1) *
        PROMETHEUS_STEP_SECONDS;
    return { startSecondsSinceEpoch, endSecondsSinceEpoch };
};
const queryPrometheusCount = async (metricName: string, queryWindow: QueryWindow): Promise<MetricData> => {
    try {
...
const convertPrometheusCountToDatadogRateSeries = (metricName: string, metricData: MetricData): void => {
    metricData.result.forEach((prometheusMetric: { metric: Record<string, string>; values: [number, string][] }) => {
...
const queryPrometheusHistogram = async (metricName: string, quantile: number, queryWindow: QueryWindow): Promise<MetricData> => {
    try {
        const response = await axios.get(PROM_QUERY_URL, {
            httpsAgent,
            params: {
...
const convertPrometheusHistogramToDatadogGuageSeries = (metricName, quantile, metricData) => {
...
const main = async () => {
    ...
    while (true) {
        ...
        await Promise.all(countMetricNames.map(async (metricName) => {
            const data = await queryPrometheusCount(metricName, generateQueryWindow());
            convertPrometheusCountToDatadogRateSeries(metricName, data);
        }));
        // Process histogram metrics
        await Promise.all(histogramMetricNames.map(async (metricName) => Promise.all(HISTOGRAM_QUANTILES.map(async (quantile) => {
            const data = await queryPrometheusHistogram(metricName, quantile, generateQueryWindow());
            convertPrometheusHistogramToDatadogGuageSeries(metricName, quantile, data);
        }))));
