import { useSendNotification } from "@marketing/hooks/useNotification";
import type { CourseProgressData } from "@marketing/lib/api/academy_api";
...
export function useAcademyContentProgress({
  ...
  const url = browserId
    ? academyApiUrl(
        `/progress?contentType=${contentType}&contentSlug=${contentSlug}&_bid=${browserId}`
      )
    : academyApiUrl(
        `/progress?contentType=${contentType}&contentSlug=${contentSlug}`
      );
...
export function useAcademyCourseProgress({
  ...
  // the actual browserId is sent via the X-Academy-Browser-Id header.
  const url = browserId
    ? academyApiUrl(`/progress/courses?_bid=${browserId}`)
    : academyApiUrl(`/progress/courses`);
  ...
  const recordAttempt = useCallback(
    async ({
      ...
      try {
        ...
        const response = await clientFetch(academyApiUrl("/progress"), {
          method: "POST",
          headers,
          body: JSON.stringify({
            contentType,
        ...
        const progressKeyPrefix = academyApiUrl("/progress");
        void globalMutate(
          (key) => typeof key === "string" && key.startsWith(progressKeyPrefix)
        );
...
export function useAcademyBackfill({
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await clientFetch(academyApiUrl("/progress/backfill"), {
          method: "POST",
