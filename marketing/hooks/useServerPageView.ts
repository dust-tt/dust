...
import { DUST_ANONYMOUS_ID_COOKIE } from "@marketing/lib/utils/anonymous_id";
import { useEffect, useRef } from "react";

/**
 * Sends a server-side page view event to `/m/api/t/pv` on each route change.
    ...
    function sendPageView() {
      ...
      if (typeof navigator.sendBeacon === "function") {
        navigator.sendBeacon(
          `${config.getApiBaseUrl()}/m/api/t/pv`,
          new Blob([body], { type: "application/json" })
        );
      } else {
        void clientFetch("/m/api/t/pv", {
          method: "POST",
