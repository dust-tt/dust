...
export const requestLogger = createMiddleware<RequestLoggerEnv>(
  async (c, next) => {
    ...
    // Tracks the peak number of concurrent Sequelize queries during this
    // request. SequelizeWithComments increments/decrements `concurrent` and
    // raises `peak` around every query (see front/lib/api/query_tracker.ts).
