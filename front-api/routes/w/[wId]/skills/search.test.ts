...
describe("GET /api/w/:wId/skills/search", () => {
  ...
  it("accepts an optional page size and cursor", async () => {
    ...
    searchSkills.mockResolvedValue(new Ok({ skills: [], nextCursor: cursor }));
    const response = await honoApp.request(
  ...
  it("tells the caller to restart an expired or mismatched cursor", async () => {
    const { workspace } = await createPrivateApiMockRequest();
    searchSkills.mockResolvedValue(new Err(new SkillSearchCursorError()));
    const response = await honoApp.request(
  ...
  it("allows admins to opt into redacted search", async () => {
    ...
    searchSkills.mockResolvedValue(new Ok({ skills: [], nextCursor: null }));
    const response = await honoApp.request(
  ...
  it("accepts ranking modes and structured filters without treating false as true", async () => {
    ...
    searchSkills.mockResolvedValue(new Ok({ skills: [], nextCursor: null }));
    const response = await honoApp.request(
      `/api/w/${workspace.sId}/skills/search?mode=management&status=active,archived&spaceIds=one,two&toolIds=tool&availability=editors,workspace_users&isDefault=false&editedByMe=true`
    );
    expect(response.status).toBe(200);
