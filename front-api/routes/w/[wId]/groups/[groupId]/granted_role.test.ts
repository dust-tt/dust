...
function grantedRoleUrl(wId: string, groupId: string) {
  return `/api/w/${wId}/groups/${groupId}/granted_role`;
}

function putGrantedRole(
  wId: string,
  groupId: string,
  body: Record<string, unknown>
) {
  return honoApp.request(grantedRoleUrl(wId, groupId), {
    method: "PUT",
...
describe("/api/w/[wId]/groups/[groupId]/granted_role", () => {
  it("returns 403 when caller is a manager (admin only)", async () => {
    ...
    const response = await putGrantedRole(workspace.sId, group.sId, {
      grantedRole: "admin",
    });
  ...
  it("lets an admin map a group to a role and clear it", async () => {
    const workspace = await WorkspaceFactory.basic();
    const group = await makeProvisionedGroup(workspace);
    ...
    const setResponse = await putGrantedRole(workspace.sId, group.sId, {
      grantedRole: "manager",
    });
    ...
    expect(reloaded.value.grantedRole).toBe("manager");

    const clearResponse = await putGrantedRole(workspace.sId, group.sId, {
      grantedRole: null,
    });
    ...
    expect(reloaded.value.grantedRole).toBeNull();
  });
  ...
  it("returns 400 on an invalid role value", async () => {
    const workspace = await WorkspaceFactory.basic();
    const group = await makeProvisionedGroup(workspace);
    await createPrivateApiMockRequest({
      method: "PUT",
  ...
  it("returns 404 when the group belongs to another workspace", async () => {
    ...
    const response = await putGrantedRole(workspace.sId, otherGroup.sId, {
      grantedRole: "admin",
    });
