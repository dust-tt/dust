...
app.put(
  ...
  async (ctx): HandlerResult<PutGroupGrantedSeatTypeResponseBody> => {
    ...
    // Captured before the mutation (setGrantedSeatType updates the instance).
    const previousSeatType = group.grantedSeatType;
    ...
    // Audit the mapping change (the admin action that drives members' seats).
    // Skip a no-op re-save.
    if (previousSeatType !== grantedSeatType) {
      void emitAuditLogEvent({
        auth,
        action: "group.granted_seat_type_updated",
        targets: [
        ...
        metadata: {
          previous_seat_type: previousSeatType ?? "none",
          new_seat_type: grantedSeatType ?? "none",
        },
      });
    }
