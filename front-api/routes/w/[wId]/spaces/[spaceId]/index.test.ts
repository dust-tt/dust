...
describe("GET /api/w/:wId/spaces/:spaceId", () => {
  ...
  it("reflects real Tools and Triggers usage on the category rows, and shows a Triggers row at all", async () => {
    ...
    // Before the fix, `.count` was never patched for `triggers`, so this row was always 0
    // and never rendered on the space landing page.
    ...
    // Before the fix, `actions.usage` was always `{ count: 0, agents: [] }` regardless of
    // real usage, since the shared loop only ever looks at `DataSourceView`s.
