...
describe("PATCH/DELETE /api/w/:wId/spaces/:spaceId/sandbox/env-vars/:id", () => {
  ...
  it("returns 404 for a missing sandbox environment variable", async () => {
    ...
    const response = await deleteEnvVar(workspace.sId, pod.sId, staleSId);
  ...
  it("promotes a config env var to an HTTPS secret", async () => {
    ...
    const data = (await response.json()) as {
      envVar: { kind: string; allowedDomains: string[] };
    };
    expect(data.envVar.kind).toBe("https_secret");
    expect(data.envVar.allowedDomains).toEqual(["api.example.com"]);
  });
