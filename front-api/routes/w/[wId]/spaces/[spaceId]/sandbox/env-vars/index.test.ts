...
describe("GET/POST /api/w/:wId/spaces/:spaceId/sandbox/env-vars", () => {
  ...
  it("lists only this pod's env vars, not workspace or other pod ones", async () => {
    ...
    const data = (await response.json()) as {
      envVars: { name: string; spaceId: string | null }[];
    };
    expect(data.envVars).toHaveLength(1);
    expect(data.envVars[0].name).toBe("DST_POD_TOKEN");
    expect(data.envVars[0].spaceId).toBe(pod.sId);
  });
  ...
  it("creates a config env var with the DST_ prefix and returns 201", async () => {
    ...
    const data = (await response.json()) as {
      envVar: { name: string; kind: string; spaceId: string | null };
      created: boolean;
    };
    expect(data.created).toBe(true);
    expect(data.envVar.name).toBe("DST_API_TOKEN");
    expect(data.envVar.kind).toBe("config");
    expect(data.envVar.spaceId).toBe(pod.sId);
