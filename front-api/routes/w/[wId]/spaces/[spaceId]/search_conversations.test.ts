...
function search(
  ...
  return honoApp.request(
    `/api/w/${workspace.sId}/spaces/${spaceId}/search_conversations${
      qs ? `?${qs}` : ""
    }`
  );
}
...
async function setupProjectSpaceWithMember() {
  ...
  return { workspace, user, auth, globalGroup, projectSpace };
}
...
describe("GET /api/w/:wId/spaces/:spaceId/search_conversations", () => {
  ...
  it("validates query parameter is not empty", async () => {
    ...
    const response = await search(workspace, projectSpace.sId, { query: "" });
  ...
  it("returns empty array when project datasource does not exist", async () => {
    ...
    const response = await search(workspace, regularSpace.sId, {
      query: "test query",
    });
  ...
  it("returns 404 for space user cannot read", async () => {
    ...
    const response = await search(workspace, unpermittedSpace.sId, {
      query: "test query",
    });
