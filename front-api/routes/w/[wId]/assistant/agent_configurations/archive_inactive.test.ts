...
describe("POST /api/w/:wId/assistant/agent_configurations/archive_inactive/preview", () => {
  ...
  it("returns 403 when the feature flag is off", async () => {
    ...
    expect(body.error.message).toMatch(/archive_inactive_agents/);
  });
  ...
  it("counts what the executor would archive, and why it would spare the rest", async () => {
    ...
    const { preview } = (await response.json()) as PreviewBody;
  ...
  it("archives nothing", async () => {
    ...
    expect(((await first.json()) as PreviewBody).preview.eligibleCount).toBe(1);
    ...
    expect(((await second.json()) as PreviewBody).preview.eligibleCount).toBe(
      1
    );
  });
});
...
describe("POST /api/w/:wId/assistant/agent_configurations/archive_inactive", () => {
  ...
  it("archives what the preview counted, and nothing else", async () => {
    ...
    const previewed = (await preview.json()) as PreviewBody;
    ...
    expect(response.status).toBe(200);
    const { archival } = (await response.json()) as ArchivalBody;

    expect(archival.archivedCount).toBe(previewed.preview.eligibleCount);
  ...
  it("archives nothing on a second call", async () => {
    ...
    expect(((await first.json()) as ArchivalBody).archival.archivedCount).toBe(
      1
    );
    ...
    const { archival } = (await second.json()) as ArchivalBody;
