...
// Mounted at /api/w/:wId/assistant/agent_configurations/:aId/observability/pods.
...
app.get(
  ...
  async (ctx) => {
    ...
    const result = await fetchPodUsageBreakdown(auth, baseQuery);
    if (result.isErr()) {
      return apiError(ctx, {
        status_code: 500,
        api_error: {
          type: "internal_server_error",
          message: `Failed to retrieve pod usage breakdown: ${fromError(result.error).toString()}`,
        },
      });
    }
