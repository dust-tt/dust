...
describe("POST /api/w/:wId/assistant/conversations/search", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("searches titles from the body and returns list items without model identifiers", async () => {
    const { workspace, auth, user } = await createPrivateApiMockRequest({
      method: "POST",
      role: "admin",
    });
    const conversation = await createConversationWithTitle(
      auth,
      {
        agentConfigurationId: GLOBAL_AGENTS_SID.DUST,
        messagesCreatedAt: [new Date()],
      },
      "Archived customer briefing",
      user.id
    );

    const response = await honoApp.request(
      `/api/w/${workspace.sId}/assistant/conversations/search`,
      {
        method: "POST",
    ...
    expect(response.status).toBe(200);
    expect(data.conversations).toHaveLength(1);
    expect(data.conversations[0].sId).toBe(conversation.sId);
    expect(data.conversations[0]).not.toHaveProperty("id");
    expect(data.hasMore).toBe(false);
    expect(data.lastValue).toEqual(expect.any(String));
  });

  it.each([
    { query: " " },
    { query: "test", limit: 101 },
  ])("rejects invalid search input: %j", async (body) => {
    const { workspace } = await createPrivateApiMockRequest({
      method: "POST",
      role: "admin",
    });
