...
app.patch(
  ...
  async (ctx) => {
    ...
    // Sync this member's seat immediately (single-user scope) so their
    // credits and credit state — and therefore whether they're still shown
    // as blocked — are up to date right away, instead of waiting for the
    // debounced workflow.
    ...
    if (syncResult.isErr()) {
      logger.warn(
        ...
        "[SeatType] Immediate seat sync after seat change failed; debounced workflow will retry"
      );
    }
