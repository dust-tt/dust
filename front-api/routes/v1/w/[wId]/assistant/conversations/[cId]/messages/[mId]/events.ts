 ...
 *     responses:
 *       200:
 *         description: SSE event stream with a `:connect` comment followed by message frames. The JSON `data` field in each message frame contains the event, discriminated by `type`.
 *         content:
 *           text/event-stream:
 *             schema:
 *               type: object
