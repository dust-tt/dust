...
/**
 * @cc [owner:flvndvd,label:product] document-ui-owned-by-viz
 ...
 */
/**
 * @cc [owner:flvndvd,label:product] document-viewer-read-only
 * Unless the Viz host enables editing, or when rendering a PDF, Document MUST disable editing,
 * formatting controls, and save callbacks regardless of its readOnly and onSave props.
 */
/**
...
export function Document({
  ...
  const { isEditable: isFrameEditable, isPdfMode } = useVizContext();
  ...
  const save = useCallback(async () => {
    ...
    setError(null);
    try {
      // The callback belongs to generated Frame code and may reject on a transport error.
  ...
  useEffect(() => {
    if (!dirty || saving || error || !editable) {
      return;
    }
