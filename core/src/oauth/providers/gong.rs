...
lazy_static! {
  static ref OAUTH_GONG_CLIENT_ID: String =
      env::var("OAUTH_GONG_CLIENT_ID").unwrap();
  static ref OAUTH_GONG_CLIENT_SECRET: String =
      env::var("OAUTH_GONG_CLIENT_SECRET").unwrap();
}
...
impl Provider for GongConnectionProvider {
  ...
  async fn finalize(
    ...
    let req = reqwest::Client::new()
      ...
      let access_token = match raw_json["access_token"].as_str() {
          Some(token) => token,
          None => Err(anyhow!(
              "Missing `access_token` in response from Gong"
          ))?,
      };
      ...
      let expires_in = match raw_json.get("expires_in") {
          Some(serde_json::Value::Number(n)) => match n.as_u64() {
              Some(n) => n,
              None => Err(anyhow!(
                  "Invalid `expires_in` in response from Gong"
              ))?,
          },
          _ => Err(anyhow!(
              "Missing `expires_in` in response from Gong"
          ))?,
      };
      ...
      let refresh_token = match raw_json["refresh_token"].as_str() {
          Some(token) => token,
          None => Err(anyhow!(
              "Missing `refresh_token` in response from Gong"
          ))?,
      };
  ...
  async fn refresh(&self, connection: &Connection) -> Result<RefreshResult, ProviderError> {
      let refresh_token = match connection.unseal_refresh_token() {
          Ok(Some(token)) => token,
          Ok(None) => Err(anyhow!(
              "Missing `refresh_token` in Gong connection"
          ))?,
          Err(e) => Err(e)?,
      };
