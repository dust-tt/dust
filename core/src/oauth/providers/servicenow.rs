...
pub struct ServicenowConnectionProvider {}

impl ServicenowConnectionProvider {
    pub fn new() -> Self {
        ServicenowConnectionProvider {}
    }

    /// Gets the ServiceNow credentials (client_id and client_secret) from the related credential
    pub async fn get_credentials(credentials: Option<Credential>) -> Result<(String, String)> {
        let credentials =
            credentials.ok_or_else(|| anyhow!("Missing credentials for ServiceNow connection"))?;
        ...
        let client_id = content
            .get("client_id")
            .and_then(|v| v.as_str())
            .ok_or_else(|| anyhow!("Missing client_id in ServiceNow credential"))?;

        let client_secret = content
            .get("client_secret")
            .and_then(|v| v.as_str())
            .ok_or_else(|| anyhow!("Missing client_secret in ServiceNow credential"))?;
...
impl Provider for ServicenowConnectionProvider {
    fn id(&self) -> ConnectionProvider {
        ConnectionProvider::Servicenow
    }

    fn reqwest_client(&self) -> reqwest::Client {
        // ServiceNow provider makes requests to user-provided instance URLs, so we use the untrusted egress proxy.
    ...
    async fn finalize(
        ...
        // Extract instance URL from connection metadata
        let servicenow_instance_url = connection
            .metadata()
            .get("servicenow_instance_url")
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                anyhow!("Missing `servicenow_instance_url` in connection metadata for ServiceNow")
            })?;

        // Get ServiceNow client_id and client_secret using the helper
        let (client_id, client_secret) = Self::get_credentials(related_credentials).await?;

        let token_endpoint = format!(
            "{}/oauth_token.do",
            servicenow_instance_url.trim_end_matches('/')
        );
        ...
        let raw_json = execute_request(ConnectionProvider::Servicenow, req)
            .await
        ...
        let access_token = match raw_json["access_token"].as_str() {
            Some(token) => token,
            None => Err(anyhow!(
                "Missing `access_token` in response from ServiceNow"
            ))?,
        };
        ...
        let expires_in_seconds = match raw_json.get("expires_in") {
            ...
            _ => Err(anyhow!("Missing `expires_in` in response from ServiceNow"))?,
        };
        let refresh_token = match raw_json["refresh_token"].as_str() {
            Some(token) => token,
            None => Err(anyhow!(
                "Missing `refresh_token` in response from ServiceNow"
            ))?,
        };
    ...
    async fn refresh(
        ...
        let refresh_token = match connection.unseal_refresh_token() {
            Ok(Some(token)) => token,
            Ok(None) => Err(anyhow!("Missing `refresh_token` in ServiceNow connection"))?,
            Err(e) => Err(e)?,
        };
    ...
    fn handle_provider_request_error(&self, error: ProviderHttpRequestError) -> ProviderError {
        match &error {
            ProviderHttpRequestError::RequestFailed {
                ...
                info!(message, is_revoked, "ServiceNow 403 error");
                if is_revoked {
