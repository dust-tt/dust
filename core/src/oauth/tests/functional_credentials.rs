...
async fn test_oauth_credentials_flow_ok() {
    ...
    let credential: CredentialExpectedResponse =
        serde_json::from_value(api_response.response.get("credential").unwrap().clone()).unwrap();
    ...
    assert_eq!(api_response.error, None);
    let retrieved_credential: CredentialExpectedResponse =
        serde_json::from_value(api_response.response.get("credential").unwrap().clone()).unwrap();
