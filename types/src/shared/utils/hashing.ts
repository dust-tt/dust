...
export function encrypt(text: string, key: string): string {
  const cipher = crypto.createCipheriv("aes-256-cbc", saltedKey(key), key.substring(0, 16));
...
export function decrypt(encrypted: string, key: string): string {
  const decipher = crypto.createDecipheriv("aes-256-cbc", saltedKey(key), key.substring(0, 16));
