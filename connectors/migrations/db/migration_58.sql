...
CREATE TABLE IF NOT EXISTS "gong_users" ("createdAt" TIMESTAMP WITH TIME ZONE NOT NULL, "updatedAt" TIMESTAMP WITH TIME ZONE NOT NULL, "email" VARCHAR(255) NOT NULL, "emailAliases" VARCHAR(255)[] NOT NULL DEFAULT ARRAY[]::VARCHAR(255)[], "firstName" VARCHAR(255), "gongId" VARCHAR(255), "isActive" BOOLEAN NOT NULL DEFAULT true, "lastName" VARCHAR(255), "title" VARCHAR(255), "connectorId" BIGINT NOT NULL REFERENCES "connectors" ("id") ON DELETE RESTRICT ON UPDATE CASCADE, "id"  BIGSERIAL , PRIMARY KEY ("id"));
