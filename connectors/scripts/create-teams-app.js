...
try {
  execSync(`cd "${buildDir}" && zip -r ../teams-app.zip manifest.json color.png outline.png`);
  ...
  console.log(`3. Update Bot Framework endpoint to: YOUR_NGROK_URL/webhooks/${process.env.WEBHOOK_SECRET || 'mywebhooksecret'}/teams_messages`);
} catch (error) {
