...
if [ -z "${CONNECTORS_DATABASE_URI:-}" ]; then
  echo "Error: CONNECTORS_DATABASE_URI must be set."
  echo "Locally: postgres://dev:dev@localhost:5432/dust_connectors"
  exit 1
fi
...
# The current CONNECTORS_DATABASE_URI is the baseline (already at production schema).
# We only need one shadow DB to materialize the current branch's models.
BASE_DSN="${CONNECTORS_DATABASE_URI%/*}"
ADMIN_DSN="${BASE_DSN}/postgres"

SHADOW_TO="dust_connectors_shadow_to_$$"
...
CONNECTORS_DATABASE_URI="${TO_DSN}" npx tsx src/admin/db.ts

echo "Computing diff..."
pg-schema-diff plan \
  --from-dsn "${CONNECTORS_DATABASE_URI}" \
  --to-dsn "${TO_DSN}" \
