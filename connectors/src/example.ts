(async () => {
  ...
  const res = await fetch("http://localhost:3002/connectors/create/notion", {
    method: "POST",
    headers: {
  ...
  if (res.status >= 200 && res.status < 300) {
    console.log("successfully created connector");
    console.log(await res.json());
  } else {
    const text = await res.text();
    console.error("error creating connector", text);
    process.exit(1);
  }
})().catch((e) => {
  console.error(e);
