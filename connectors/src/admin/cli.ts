...
const notion = async (command: string, args: parseArgs.ParsedArgs) => {
  switch (command) {
    ...
    case "upsert-page": {
      ...
      logger.info("[Poke Admin] Upserting page", {
        pageId,
        commandType: "notion",
        command,
        args,
    ...
    case "upsert-database": {
      ...
      logger.info("[Poke Admin] Upserting database", {
        databaseId,
        commandType: "notion",
        command,
        args,
    ...
    case "me": {
      ...
      logger.info("[Poke Admin] " + JSON.stringify(me.bot.owner), {
        commandType: "notion",
        command,
        args,
      });
    ...
    case "stop-all-garbage-collectors": {
      ...
      logger.info("[Poke Admin] Stopping all notion garbage collectors", {
      ...
      for (const connector of connectors) {
        logger.info("[Poke Admin] Stopping notion garbage collector", {
          connectorId: connector.id,
          commandType: "notion",
          command,
          args,
...
const temporal = async (command: string, args: parseArgs.ParsedArgs) => {
  switch (command) {
    case "check-queue": {
      ...
      logger.info("[Poke Admin] DescribeTqRes", {
      	describeTqRes,
        commandType: "temporal",
        command,
        args,
      });
      break;
    ...
    case "find-unprocessed-workflows": {
      ...
      for (const q of queues) {
        logger.info("[Poke Admin] looking at queue", {
          q,
          commandType: "temporal",
          command,
          args,
