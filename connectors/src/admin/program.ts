...
export interface CliMainCommandManifest {
  description: string;
  subcommands: string[];
  options: CliOptionDef[];
}

// Duck-typed view of what Commander stores internally for each registered command.
// Commander 9 uses `options` (public array) and `_args` (internal argument store).
interface IntrospectableCommand {
  ...
  options: Array<{
    ...
    description: string;
    flags: string;
    parseArg: ((value: string, previous: unknown) => unknown) | undefined;
  }>;
  _args: Array<{ argChoices: string[] | null }>;
}

function isIntrospectable(cmd: unknown): cmd is IntrospectableCommand {
  if (cmd === null || typeof cmd !== "object") return false;
  return (
    typeof Reflect.get(cmd, "name") === "function" &&
    typeof Reflect.get(cmd, "description") === "function" &&
    Array.isArray(Reflect.get(cmd, "options")) &&
    Array.isArray(Reflect.get(cmd, "_args"))
  );
}

export function buildManifest(): Record<string, CliMainCommandManifest> {
  const manifest: Record<string, CliMainCommandManifest> = {};
  for (const rawCmd of program.commands) {
    if (!isIntrospectable(rawCmd)) continue;
    const subcommands = rawCmd._args[0]?.argChoices ?? [];
    const options: CliOptionDef[] = rawCmd.options
      .filter((opt) => opt.name() !== "help")
      .map((opt) => {
        let type: CliOptionType;
        if (!opt.flags.includes("<") && !opt.flags.includes("[")) {
          type = "boolean";
        } else if (opt.parseArg !== undefined) {
          type = "number";
        } else {
          type = "string";
        }
        return { name: opt.name(), description: opt.description, type };
      });
    manifest[rawCmd.name()] = {
